...
function renderSearchConditionChips(){
  ...
  if(isTypeSearchMode()){
    ...
    if(tags.length)chips.push(renderSelectedTagConditionChipsHtml());
...
function collectTypeSearchCatalogFrom(field){const map=new Map();getTypeSearchIndexItems().forEach(item=>(Array.isArray(item?.[field])?item[field]:[]).forEach(ref=>{const id=norm(ref?.featureId||'');if(!id||map.has(id))return;map.set(id,{featureId:id,canonicalFeatureKey:norm(ref?.canonicalFeatureKey||id),featureDomain:norm(ref?.featureDomain||''),label:norm(ref?.label||ref?.statusEffectName||id),featureType:norm(ref?.featureType||''),groupLabel:norm(ref?.groupLabel||''),groupKey:norm(ref?.groupKey||'')});}));return [...map.values()].sort((a,b)=>{const ga=norm(a.groupLabel||'');const gb=norm(b.groupLabel||'');if(ga!==gb)return ga.localeCompare(gb,'ja');return a.label.localeCompare(b.label,'ja');});}
...
function renderTypeSearchPresetInfo(){const info=document.getElementById('typeSearchPresetInfo');if(!info)return;const wasOpen=!!info.open;const preset=getActiveTypeSearchPreset();if(!preset){info.className='type-search-preset-info is-empty';info.innerHTML='';info.hidden=true;info.open=false;return;}const conditions=getTypeSearchPresetConditions(preset);const counts=summarizeTypeSearchImportance(conditions);const dirty=state.typeSearchPresetDirty?'<span class="type-search-preset-dirty">条件編集済み</span>':'';const groupHtml=['core','recommended','support'].map(importance=>{const labels=conditions.filter(v=>normalizeTypeSearchImportance(v?.importance)===importance).map(v=>norm(v?.label||v?.featureId||'')).filter(Boolean);if(!labels.length)return'';return `<span class="type-search-preset-condition-group"><span class="type-search-importance-chip is-${importance}">${esc(getTypeSearchImportanceLabel(importance))}</span> ${esc(labels.join(' / '))}</span>`;}).join('');info.hidden=false;info.className='type-search-preset-info';info.title=norm(preset.description||'');info.innerHTML=`<summary><span class="type-search-preset-title">${esc(preset.typeName||preset.typeId||'')}${dirty}</span>：中核${counts.core||0}・推奨${counts.recommended||0}・補助${counts.support||0}</summary><div class="type-search-preset-detail">${groupHtml}</div>`;info.open=wasOpen;}
...
// FIX[HADO-3.0.0.0-UPDATE09.5.60-SEARCH-MODE-ISOLATION]:
// 通常・状態変化・型検索は、それぞれの検索条件、対象カテゴリ、選択中詳細を独立して保持する。
// DOMを隠すだけでは非表示の通常キーワードが状態変化検索へ混入するため、モード切替時に
// 現在モードを保存し、移動先モードのruntime状態だけを復元する。
function cloneSearchModeCategories(source){const out={};Object.keys(state.activeCategories||{}).forEach(key=>{out[key]=!!source?.[key];});return out;}
function getSearchModeSelectionSnapshot(){const item=state.selectedItem;return item?{item,category:detailCategory(item),name:getItemDisplayName(item),label:state.selectedLabel||getCategoryLabel(detailCategory(item)),detailActiveTab:state.detailActiveTab||getDetailInitialTabForItem(item)}:null;}
...
function restoreSearchModeContext(mode,context){
  ...
  state.searchResultRenderLimit=Number(context.resultRenderLimit)||100;
  state.quickStatusEffectOwnerFilter=null;state._quickOwnerRowsCache=null;
  ...
  if(mode==='status'){
    ...
    const groupSelect=document.getElementById('quickStatusEffectGroupSelect');if(groupSelect)groupSelect.value=context.quickGroup||'all';
    refreshQuickStatusEffectOptions(context.quickEffect||'');
    const effectSelect=document.getElementById('quickStatusEffectSelect');if(effectSelect)effectSelect.value=context.quickEffect||'';
    const resetBtn=document.getElementById('quickStatusEffectResetBtn');if(resetBtn)resetBtn.classList.toggle('is-active',!!state.quickStatusEffectOwnerFilter);
  }else if(mode==='type'){
    state.typeSearchSelectedPresetId=context.typePresetId||'';state.typeSearchPresetDirty=!!context.typePresetDirty;state.typeSearchSelectedStatusEffectIds=[...(context.typeStatusIds||[])];state.typeSearchSelectedFeatureIds=[...(context.typeFeatureIds||[])];
    const presetSelect=document.getElementById('typeSearchPresetSelect');if(presetSelect)presetSelect.value=state.typeSearchSelectedPresetId;
  }
  const selected=resolveSearchModeSelection(context.selection);state.selectedItem=selected;state.selectedLabel=selected?(context.selection?.label||getCategoryLabel(detailCategory(selected))):'';state.detailActiveTab=selected?(context.selection?.detailActiveTab||getDetailInitialTabForItem(selected)):'';
...
function updateSearchModeUi(){
  const mode=norm(state.searchMode||'normal');
  let activeModeButton=null;
  [['normal','searchModeNormalBtn'],['status','searchModeStatusBtn'],['type','searchModeTypeBtn']].forEach(([key,id])=>{
    const btn=document.getElementById(id);if(!btn)return;
    const active=mode===key;btn.classList.toggle('is-active',active);btn.setAttribute('aria-selected',active?'true':'false');if(active)activeModeButton=btn;
  });
  const modeBar=document.getElementById('searchModeBar');if(window.HADO_TABS?.sync)window.HADO_TABS.sync(modeBar,activeModeButton);
  const statusBar=document.getElementById('searchPresetBar');if(statusBar){statusBar.hidden=mode!=='status';statusBar.classList.toggle('has-tag-filter',mode==='status');}
  const typePanel=document.getElementById('typeSearchPanel');if(typePanel)typePanel.hidden=mode!=='type';
  const queryRow=document.getElementById('searchQueryRow');if(queryRow)queryRow.hidden=mode==='status';
  const inputRow=document.getElementById('normalSearchInputRow');if(inputRow)inputRow.hidden=mode!=='normal';
  const tagWrap=document.getElementById('tagSearchWrap');
  if(tagWrap){
    tagWrap.hidden=false;
    const target=(mode==='status'&&statusBar)?statusBar:queryRow;
    if(target&&tagWrap.parentElement!==target)target.appendChild(tagWrap);
  }
  if(els.nameOnlySearchToggle)els.nameOnlySearchToggle.disabled=mode!=='normal';
  if(els.clearKeywordBtn)els.clearKeywordBtn.hidden=mode!=='normal';
  if(els.copyParamResultsBtn)els.copyParamResultsBtn.hidden=mode==='type';
  document.querySelectorAll('#categoryBar button[data-category]').forEach(btn=>{const hidden=mode==='type'&&!TYPE_SEARCH_ALLOWED_CATEGORIES.includes(btn.dataset.category||'');btn.classList.toggle('type-search-category-hidden',hidden);btn.disabled=hidden;});
  renderTagSearchControls();renderTypeSearchSelectedConditions();renderSearchConditionChips();
}
...
function buildTypeSearchMatchesForIndexItem(indexItem,phase=null){const selectedStatus=new Set((state.typeSearchSelectedStatusEffectIds||[]).map(norm));const selectedFeatures=new Set((state.typeSearchSelectedFeatureIds||[]).map(norm));const hits=[];const add=(ref,kind)=>{const id=norm(ref?.featureId||'');const canonicalFeatureId=norm(ref?.canonicalFeatureKey||getTypeSearchConditionCanonicalId(id));if(!id||hits.some(x=>x.canonicalFeatureId===canonicalFeatureId))return;hits.push({featureId:id,canonicalFeatureId,sourceEntityKey:norm(ref?.sourceEntityKey||indexItem?.sourceEntityKey||''),sourcePartType:norm(ref?.sourcePartType||''),featureDomain:norm(ref?.featureDomain||''),label:norm(ref?.label||ref?.statusEffectName||id),kind,importance:getTypeSearchConditionImportance(id),source:norm(ref?.sourcePartType||ref?.source||''),groupLabel:norm(ref?.groupLabel||''),matchedText:norm(ref?.matchedText||ref?.rawText||'')});};let t=performance.now();(Array.isArray(indexItem?.statusEffectRefs)?indexItem.statusEffectRefs:[]).forEach(ref=>{if(selectedStatus.has(norm(ref?.featureId||'')))add(ref,'状態変化');});if(phase)phase.statusEffectMatchMs=(phase.statusEffectMatchMs||0)+(performance.now()-t);t=performance.now();(Array.isArray(indexItem?.typeFeatures)?indexItem.typeFeatures:[]).forEach(ref=>{if(selectedFeatures.has(norm(ref?.featureId||'')))add(ref,'型要素');});if(phase)phase.featureMatchMs=(phase.featureMatchMs||0)+(performance.now()-t);const bridge=(typeof window!=='undefined'?window:globalThis).HADO_CLAUSE_SURFACE_BRIDGE;return bridge&&typeof bridge.annotateTypeSearchHits==='function'?bridge.annotateTypeSearchHits(indexItem?.category,indexItem?.name,hits):hits;}
...
function renderTypeSearchResultList(rows){if(!els.results)return;els.results.innerHTML='';const renderLimit=getSearchResultRenderLimit(rows,'',false);rows.slice(0,renderLimit).forEach(row=>{const li=document.createElement('li');if(state.selectedItem===row.item)li.classList.add('active');const displayName=getResultCardDisplayName(row);const subTitle=getResultCardSubtitle(row);const saved=(row.key==='generals'||row.key==='equipments')&&isSavedName(row.key,displayName);const main=document.createElement('div');main.className='search-result-card-main';const reasons=formatTypeSearchReasonsHtml(row.typeSearchMatches||[]);const matchedTags=renderMatchedSelectedTagsHtml(row.item);main.innerHTML=`<div class="search-result-top"><span class="search-result-category">${esc(row.label)}</span><span class="search-result-title">${highlightSelectedTagTextHtml(displayName,row.item)}</span><span class="search-result-metric">${highlightSelectedTagTextHtml(row.metric?.display||'',row.item)}</span></div>${subTitle?`<div class="search-result-meta">${highlightSelectedTagTextHtml(subTitle,row.item)}</div>`:''}${matchedTags?`<div class="search-result-tag-matches">${matchedTags}</div>`:''}${reasons?`<div class="type-search-result-reasons"><strong>一致理由：</strong>${reasons}</div>`:''}`;main.addEventListener('click',()=>selectItemAndRender(row.item,row.label,{reason:'type-search-result-click',preserveListScroll:true}));li.appendChild(main);if(row.key==='generals'||row.key==='equipments'){const btn=document.createElement('button');btn.type='button';btn.className=`save-star ${saved?'is-saved':''}`;btn.textContent=saved?'★':'☆';btn.addEventListener('click',e=>{e.stopPropagation();toggleSavedName(row.key,displayName);});li.appendChild(btn);}els.results.appendChild(li);});if(renderLimit<rows.length){const moreLi=document.createElement('li');moreLi.className='search-result-more-row';const btn=document.createElement('button');btn.type='button';btn.className='copy-btn';btn.textContent=`さらに表示（${renderLimit}/${rows.length}件）`;btn.addEventListener('click',e=>{e.stopPropagation();increaseSearchResultRenderLimit();});moreLi.appendChild(btn);els.results.appendChild(moreLi);}return renderLimit;}
function renderTypeSearchResults(){const searchProgressSeq=beginSearchProgressIndicator();const started=performance.now();const previousSelectedItem=state.selectedItem;const conditionCount=getTypeSearchSelectedConditionCount();const activeKeys=getTypeSearchActiveCategoryKeys();const selectedTags=[...(state.selectedTags||[])];const cacheKey=getTypeSearchResultCacheKey();const cacheMap=getTypeSearchResultCacheMap();let rows=[];let stats={};TYPE_SEARCH_ALLOWED_CATEGORIES.forEach(key=>{stats[key]={active:activeKeys.includes(key),total:0,matched:0};});let phase={candidateCount:0,afterSavedFilterCount:0,afterTagFilterCount:0,matchedCount:0,savedFilteredOutCount:0,tagFilteredOutCount:0,savedFilterMs:0,tagFilterMs:0,statusEffectMatchMs:0,featureMatchMs:0,sortMs:0,cacheLookupMs:0,cacheStoreMs:0,rowsBuildMs:0,importanceMs:0,conditionChipsMs:0,selectRenderMs:0,domRenderMs:0,diagnosticSnapshotMs:0,responsiveSnapshotMs:0,renderDetailMs:0,renderDetailRan:false,cacheInvalidationReason:''};let cacheHit=false;let cached=null;if(conditionCount&&activeKeys.length){let t=performance.now();cached=getCachedTypeSearchResult(cacheKey);phase.cacheLookupMs+=performance.now()-t;if(cached){cacheHit=true;rows=cached.rows||[];stats=cached.stats||stats;phase={...phase,...(cached.phase||{}),cacheLookupMs:phase.cacheLookupMs};state.typeSearchCacheStats=state.typeSearchCacheStats||{hit:0,miss:0,invalidations:0,store:0};state.typeSearchCacheStats.hit=(state.typeSearchCacheStats.hit||0)+1;}else{state.typeSearchCacheStats=state.typeSearchCacheStats||{hit:0,miss:0,invalidations:0,store:0};state.typeSearchCacheStats.miss=(state.typeSearchCacheStats.miss||0)+1;getTypeSearchIndexItems().forEach(indexItem=>{const key=norm(indexItem?.category||'');if(!activeKeys.includes(key))return;stats[key].total++;phase.candidateCount++;const item=findItemByCategoryAndName(key,indexItem?.name||'');if(!item)return;let t=performance.now();const savedOk=itemMatchesTypeSearchViewMode(item,key);phase.savedFilterMs+=performance.now()-t;if(!savedOk){phase.savedFilteredOutCount++;return;}phase.afterSavedFilterCount++;t=performance.now();const tagOk=matchesSelectedTags(item);phase.tagFilterMs+=performance.now()-t;if(!tagOk){phase.tagFilteredOutCount++;return;}phase.afterTagFilterCount++;let rb=performance.now();const hits=buildTypeSearchMatchesForIndexItem(indexItem,phase);phase.rowsBuildMs+=performance.now()-rb;if(!hits.length)return;let ib=performance.now();const importance=summarizeTypeSearchMatches(hits);phase.importanceMs+=performance.now()-ib;stats[key].matched++;phase.matchedCount++;rows.push({key,label:indexItem?.categoryLabel||DATASET_LABELS[key]||key,item,typeSearchMatches:hits,typeSearchImportance:importance,metric:{display:`一致 ${hits.length}/${conditionCount}（中核${importance.core}・推奨${importance.recommended}・補助${importance.support}${importance.manual?`・手動${importance.manual}`:''}）`,value:hits.length,reasonText:formatTypeSearchReasonText(hits),typeSearch:true}});});let t=performance.now();rows.sort((a,b)=>{const ai=a.typeSearchImportance||{};const bi=b.typeSearchImportance||{};let d=(bi.core||0)-(ai.core||0);if(d)return d;d=(bi.total||0)-(ai.total||0);if(d)return d;d=(bi.recommended||0)-(ai.recommended||0);if(d)return d;d=(bi.support||0)-(ai.support||0);if(d)return d;const c=(TYPE_SEARCH_CATEGORY_ORDER[a.key]??9)-(TYPE_SEARCH_CATEGORY_ORDER[b.key]??9);if(c)return c;return getItemDisplayName(a.item).localeCompare(getItemDisplayName(b.item),'ja');});phase.sortMs=performance.now()-t;{let cs=performance.now();storeTypeSearchResultCache(cacheKey,rows,stats,phase);phase.cacheStoreMs+=performance.now()-cs;}}}if(rows.length){const current=state.selectedItem?rows.find(row=>row.item===state.selectedItem):null;if(!current){state.selectedItem=rows[0].item;state.selectedLabel=rows[0].label;state.detailActiveTab=getDetailInitialTabForItem(rows[0].item);}}else{state.selectedItem=null;state.selectedLabel='';state.detailActiveTab='';}state.lastResultRows=rows;updateResultNavigationButtons();let t=performance.now();renderResultSelect(rows);phase.selectRenderMs=performance.now()-t;t=performance.now();const renderLimit=renderTypeSearchResultList(rows);phase.domRenderMs=performance.now()-t;let finalText='';if(!conditionCount)finalText='型検索：型プリセット、状態変化、または型要素を選択してください。';else if(!activeKeys.length)finalText='型検索：対象カテゴリを選択してください。';else finalText=`ヒット件数：${rows.length}件`;finishSearchProgressIndicator(searchProgressSeq,finalText);t=performance.now();renderSearchConditionChips();phase.conditionChipsMs=performance.now()-t;t=performance.now();debugResponsiveSnapshot('typeSearch:after-render');phase.responsiveSnapshotMs=performance.now()-t;const round=v=>Number((Number(v)||0).toFixed(1));const profile={timestamp:debugTimestamp(),mode:'type',presetId:state.typeSearchSelectedPresetId||'',presetDirty:!!state.typeSearchPresetDirty,conditionCount,statusEffectIds:[...(state.typeSearchSelectedStatusEffectIds||[])],featureIds:[...(state.typeSearchSelectedFeatureIds||[])],selectedTags,activeCategories:activeKeys,viewMode:state.viewMode,savedModePolicy:getTypeSearchSavedModePolicy(),cacheHit,cacheKey,cacheSize:cacheMap.size,cacheSeq:state.typeSearchCacheSeq||0,cacheStats:safeCloneForDebug(state.typeSearchCacheStats||{}),candidateCount:phase.candidateCount,afterSavedFilterCount:phase.afterSavedFilterCount,afterTagFilterCount:phase.afterTagFilterCount,savedFilteredOutCount:phase.savedFilteredOutCount,tagFilteredOutCount:phase.tagFilteredOutCount,resultCount:rows.length,renderedCount:renderLimit,statusEffectMatchMs:round(phase.statusEffectMatchMs),featureMatchMs:round(phase.featureMatchMs),savedFilterMs:round(phase.savedFilterMs),tagFilterMs:round(phase.tagFilterMs),sortMs:round(phase.sortMs),cacheLookupMs:round(phase.cacheLookupMs),cacheStoreMs:round(phase.cacheStoreMs),rowsBuildMs:round(phase.rowsBuildMs),importanceMs:round(phase.importanceMs),conditionChipsMs:round(phase.conditionChipsMs),selectRenderMs:round(phase.selectRenderMs),domRenderMs:round(phase.domRenderMs),diagnosticSnapshotMs:round(phase.diagnosticSnapshotMs),responsiveSnapshotMs:round(phase.responsiveSnapshotMs),renderDetailRan:!!phase.renderDetailRan,renderDetailMs:round(phase.renderDetailMs),cacheInvalidationReason:state.typeSearchLastInvalidationReason||'',topRows:rows.slice(0,10).map(row=>({category:row.key,name:getItemDisplayName(row.item),importance:row.typeSearchImportance,reasons:formatTypeSearchReasonText(row.typeSearchMatches)})),categoryStats:stats,history:{index:state.operationHistoryIndex,size:state.operationHistory.length},totalMs:round(performance.now()-started),policy:'HADO-2.9.4.0: 型検索はJSON正本プリセットを利用し、状態変化と型要素はOR、属性タグはグループ内OR・グループ間AND。保存データ表示では武将・装備のみ所有登録で絞り込み、兵器・軍馬技能は全件候補。結果はdataSetId・条件集合・タグ・カテゴリ・表示範囲・保存索引世代でキャッシュする。'};state.diagnostics.typeSearch=profile;state.diagnostics.typeSearchCache={timestamp:profile.timestamp,cacheHit,cacheKey,cacheSize:cacheMap.size,cacheSeq:profile.cacheSeq,cacheStats:profile.cacheStats,lastInvalidationReason:state.typeSearchLastInvalidationReason||'',savedModePolicy:profile.savedModePolicy};const knownKeys=['statusEffectMatchMs','featureMatchMs','savedFilterMs','tagFilterMs','sortMs','cacheLookupMs','cacheStoreMs','rowsBuildMs','importanceMs','conditionChipsMs','selectRenderMs','domRenderMs','diagnosticSnapshotMs','responsiveSnapshotMs','renderDetailMs'];profile.measuredKnownMs=round(knownKeys.reduce((sum,key)=>sum+(Number(profile[key])||0),0));profile.unmeasuredMs=round(Math.max(0,profile.totalMs-profile.measuredKnownMs));profile.unmeasuredMsWarning=profile.totalMs>200||profile.unmeasuredMs>50;state.lastSearchProfile=profile;t=performance.now();updateSearchDiagnosticSnapshot(profile);phase.diagnosticSnapshotMs=performance.now()-t;profile.diagnosticSnapshotMs=round(phase.diagnosticSnapshotMs);profile.measuredKnownMs=round(knownKeys.reduce((sum,key)=>sum+(Number(profile[key])||0),0));profile.unmeasuredMs=round(Math.max(0,profile.totalMs-profile.measuredKnownMs));profile.unmeasuredMsWarning=profile.totalMs>200||profile.unmeasuredMs>50;debugLog('typeSearch:profile',profile);debugLog('typeSearch:cache',state.diagnostics.typeSearchCache);if(previousSelectedItem!==state.selectedItem){t=performance.now();phase.renderDetailRan=true;renderDetail();phase.renderDetailMs=performance.now()-t;if(state.lastSearchProfile){state.lastSearchProfile.renderDetailRan=true;state.lastSearchProfile.renderDetailMs=round(phase.renderDetailMs);}}return rows;}
...
function renderSearchResults(){
  ...
  const normalSearchMode=norm(state.searchMode||'normal')==='normal';
  const nameOnlySearch=normalSearchMode&&isNameOnlySearch();
  state.nameOnlySearch=nameOnlySearch;
  const searchContext=!normalSearchMode||nameOnlySearch?null:currentSearchContext();
  debugLog('searchContext',{nameOnlySearch,context:searchContext});
  const effectiveKeyword=normalSearchMode?norm(nameOnlySearch?currentKeyword():((searchContext?.base)||currentKeyword())):'';
  ...
  const datasets=[['generals','武将',state.generals],['tactics','戦法',state.tactics],['skills','技能',state.skills],['troopSkills','兵科',state.troopSkills],['equipments','装備',state.equipments],['statusEffects','状態変化',state.statusEffects],['siegeWeapons','兵器',state.siegeWeapons],['ethnicArmaments','武装',state.ethnicArmaments],['formations','陣形',state.formationMasters],['warhorses','名馬',getSearchWarhorseItems()],['warhorseSkills','軍馬技能',state.warhorseSkills]];
  ...
  if(rows.length){
    ...
    const exactRow=normalSearchMode&&q?rows.find(row=>norm(getItemDisplayName(row.item)).toLowerCase()===q):null;
    if(pendingRow){
  ...
  }else if(state.selectedItem&&!(state.quickStatusEffectOwnerFilter&&(!state._quickOwnerRowsCache||state._quickOwnerRowsCache.pending))){
    state.selectedItem=null;
    state.selectedLabel='';
  }
  ...
  const renderLimit=getSearchResultRenderLimit(rows,q,nameOnlySearch);rows.slice(0,renderLimit).forEach(row=>{const li=document.createElement('li');if(state.selectedItem===row.item)li.classList.add('active');const displayName=getResultCardDisplayName(row);const subTitle=getResultCardSubtitle(row);const metricLabel=row.metric?row.metric.display:'';const useCompactCard=!!getResultCardIndexForRow(row)&&!metricLabel&&!q;const tacticAttackBadges=useCompactCard?'':((row.key==='tactics'||row.key==='generals')?renderTacticAttackResultBadges(row.item):'');const saved=(row.key==='generals'||row.key==='equipments')&&isSavedName(row.key,displayName);const cardBadges=getResultCardBadgesHtml(row);const matchedTags=renderMatchedSelectedTagsHtml(row.item);const main=document.createElement('div');main.className='search-result-card-main';const reasonText=row.metric&&row.metric.reasonText?row.metric.reasonText:'';main.innerHTML=`<div class="search-result-top"><span class="search-result-category">${esc(row.label)}</span><span class="search-result-title">${highlightSelectedTagTextHtml(displayName,row.item)}</span>${metricLabel?`<span class="search-result-metric">${highlightSelectedTagTextHtml(metricLabel,row.item)}</span>`:''}</div>${subTitle?`<div class="search-result-meta">${highlightSelectedTagTextHtml(subTitle,row.item)}</div>`:''}${matchedTags?`<div class="search-result-tag-matches">${matchedTags}</div>`:''}${cardBadges?`<div class="search-result-subrow">${cardBadges}</div>`:''}${reasonText?`<div class="search-result-reason">${highlightSelectedTagTextHtml(reasonText,row.item)}</div>`:''}${tacticAttackBadges?`<div class="search-result-subrow">${tacticAttackBadges}</div>`:''}`;if(metricLabel){domMetricRowCount++;if(domMetricSamples.length<10)domMetricSamples.push({name:norm(displayName),metricLabel,html:main.innerHTML});}main.addEventListener('click',()=>{selectItemAndRender(row.item,row.label,{reason:'result-click',preserveListScroll:true});});li.appendChild(main);if(row.key==='generals'||row.key==='equipments'){const btn=document.createElement('button');btn.type='button';btn.className=`save-star ${saved?'is-saved':''}`;btn.textContent=saved?'★':'☆';btn.addEventListener('click',e=>{e.stopPropagation();toggleSavedName(row.key,displayName);});li.appendChild(btn);}els.results.appendChild(li);});if(renderLimit<rows.length){const moreLi=document.createElement('li');moreLi.className='search-result-more-row';const btn=document.createElement('button');btn.type='button';btn.className='copy-btn';btn.textContent=`さらに表示（${renderLimit}/${rows.length}件）`;btn.addEventListener('click',e=>{e.stopPropagation();increaseSearchResultRenderLimit();});moreLi.appendChild(btn);els.results.appendChild(moreLi);}if(quickWaitingForCategory){els.results.innerHTML=`<li class="search-initial-empty"><div class="detail-empty">状態変化フィルター「${esc(state.quickStatusEffectOwnerFilter?.label||'')}」を選択中です。対象カテゴリを選択すると絞り込みます。</div></li>`;}
...
function setupCategoryButtons(){[['generals','武将'],['tactics','戦法'],['skills','技能'],['troopSkills','兵科'],['equipments','装備'],['statusEffects','状態変化'],['siegeWeapons','兵器'],['ethnicArmaments','武装'],['formations','陣形'],['warhorses','名馬'],['warhorseSkills','軍馬技能']].forEach(([key,label])=>{const btn=document.createElement('button');btn.type='button';btn.dataset.category=key;btn.textContent=label;btn.addEventListener('click',()=>toggleCategory(key));els.categoryBar.appendChild(btn);});if(els.selectAllCategoriesBtn)els.selectAllCategoriesBtn.addEventListener('click',selectAllCategories);if(els.clearAllCategoriesBtn)els.clearAllCategoriesBtn.addEventListener('click',clearAllCategories);updateCategoryStyles();}
