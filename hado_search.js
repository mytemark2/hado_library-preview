...
function buildTypeSearchMatchesForIndexItem(indexItem,phase=null){const selectedStatus=new Set((state.typeSearchSelectedStatusEffectIds||[]).map(norm));const selectedFeatures=new Set((state.typeSearchSelectedFeatureIds||[]).map(norm));const hits=[];const add=(ref,kind)=>{const id=norm(ref?.featureId||'');const canonicalFeatureId=norm(ref?.canonicalFeatureKey||getTypeSearchConditionCanonicalId(id));if(!id||hits.some(x=>x.canonicalFeatureId===canonicalFeatureId))return;hits.push({featureId:id,canonicalFeatureId,sourceEntityKey:norm(ref?.sourceEntityKey||indexItem?.sourceEntityKey||''),sourcePartType:norm(ref?.sourcePartType||''),featureDomain:norm(ref?.featureDomain||''),label:norm(ref?.label||ref?.statusEffectName||id),kind,importance:getTypeSearchConditionImportance(id),source:norm(ref?.sourcePartType||ref?.source||''),groupLabel:norm(ref?.groupLabel||'')});};let t=performance.now();(Array.isArray(indexItem?.statusEffectRefs)?indexItem.statusEffectRefs:[]).forEach(ref=>{if(selectedStatus.has(norm(ref?.featureId||'')))add(ref,'状態変化');});if(phase)phase.statusEffectMatchMs=(phase.statusEffectMatchMs||0)+(performance.now()-t);t=performance.now();(Array.isArray(indexItem?.typeFeatures)?indexItem.typeFeatures:[]).forEach(ref=>{if(selectedFeatures.has(norm(ref?.featureId||'')))add(ref,'型要素');});if(phase)phase.featureMatchMs=(phase.featureMatchMs||0)+(performance.now()-t);return hits;}
