...
(() => {
  ...
  function typeCard(row) { const rule=state.data.scoreRules.find((t)=>t.typeId===row.typeId); if(!rule)return ''; const r=scoreType(state.mainGeneral,rule); return `<button class="hte-item ${state.typeId===rule.typeId?'active':''}" data-type-id="${esc(rule.typeId)}"><span class="hte-item-role ${row.role==='primary'?'primary':''}">${esc(row.roleLabel||'型')}</span><div class="hte-title">${esc(rule.typeName)}</div>${rule.description?`<div class="hte-reason">${esc(rule.description)}</div>`:''}${row.reason?`<div class="hte-reason"><strong>${esc(row.roleLabel==='主軸型'?'主軸理由':row.roleLabel==='補助型'?'補助理由':'推奨理由')}:</strong> ${esc(row.reason)}</div>`:''}<div class="hte-score">主将適合スコア: ${r.score==null?'未採点':`${r.score}/10`}${r.matched.length?` / ${r.matched.map((m)=>esc(m.label)).join('、')}`:''}</div></button>`; }
  function renderTypeStep() { if(state.mode==='type'){ const rows=state.data.scoreRules.map((v)=>({typeId:v.typeId,role:'direct',roleLabel:'型を直接選択',reason:v.description||''})); return `<div class="hte-card"><div class="hte-title">型を直接選択</div><div class="hte-list">${rows.map(typeCard).join('')}</div><div class="hte-note">適合スコアは選択中の型の5項目を同じ比率で判定した10段階表示です。技能Lvは採点しません。異なる型同士の順位付けには使いません。</div></div>`; } const rows=purposeRows(purpose()),primary=rows.filter((v)=>v.role==='primary'),secondary=rows.filter((v)=>v.role!=='primary'); return `<div class="hte-card"><div class="hte-title">${esc(purpose()?.purposeName||'目的')}に使う型を選択</div><div class="hte-reason">${esc(purpose()?.summary||'')}</div><div class="hte-group-title">主軸型</div><div class="hte-list">${primary.map(typeCard).join('')}</div><div class="hte-group-title">補助型</div><div class="hte-list">${secondary.map(typeCard).join('')}</div><div class="hte-note">適合スコアは選択中の型の5項目を同じ比率で判定した10段階表示です。技能Lvは採点しません。異なる型同士の順位付けには使いません。</div></div>`; }
