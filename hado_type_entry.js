/* HADO app 3.0.0.0 Update09.3.5: seven-purpose wizard with count-based suitability score */
(() => {
  'use strict';
  ...
  function renderTypeStep() { if(state.mode==='type'){ const rows=state.data.scoreRules.map((v)=>({typeId:v.typeId,role:'direct',roleLabel:'型を直接選択',reason:v.description||''})); return `<div class="hte-card"><div class="hte-title">型を直接選択</div><div class="hte-list">${rows.map(typeCard).join('')}</div><div class="hte-note">適合スコアは型の5条件に一致する状態変化率項目数の件数表示です。条件別の項目数を表示します。技能Lvは採点しません。異なる型同士の順位付けには使いません。</div></div>`; } const rows=purposeRows(purpose()),primary=rows.filter((v)=>v.role==='primary'),secondary=rows.filter((v)=>v.role!=='primary'); return `<div class="hte-card"><div class="hte-title">${esc(purpose()?.purposeName||'目的')}に使う型を選択</div><div class="hte-reason">${esc(purpose()?.summary||'')}</div><div class="hte-group-title">主軸型</div><div class="hte-list">${primary.map(typeCard).join('')}</div><div class="hte-group-title">補助型</div><div class="hte-list">${secondary.map(typeCard).join('')}</div><div class="hte-note">適合スコアは型の5条件に一致する状態変化率項目数の件数表示です。条件別の項目数を表示します。技能Lvは採点しません。異なる型同士の順位付けには使いません。</div></div>`; }
