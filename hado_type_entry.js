...
(() => {
  ...
  function saveSelection() {
    ...
    const openCandidates = () => {
      if (window.HadoTypeCandidates && typeof window.HadoTypeCandidates.open === 'function') {
        window.HadoTypeCandidates.open({ source: 'type-entry-save' });
      } else {
  ...
  function renderConfirmStep() { const row=purposeRows(purpose()).find((v)=>v.typeId===state.typeId)||null; return `<div class="hte-card"><div class="hte-title">選択内容を確認</div><div class="hte-confirm"><div class="hte-confirm-row"><div class="hte-confirm-label">選び方</div><div>${esc(state.mode === 'main' ? '主将から考える' : state.mode === 'purpose' ? '目的から考える' : '型を直接選ぶ')}</div></div><div class="hte-confirm-row"><div class="hte-confirm-label">主将</div><div>${esc(state.mainGeneral?.displayName || state.mainGeneral?.name || '指定なし')}</div></div><div class="hte-confirm-row"><div class="hte-confirm-label">目的</div><div>${esc(purpose()?.purposeName || '指定なし')}</div></div><div class="hte-confirm-row"><div class="hte-confirm-label">型</div><div>${esc(typeRule()?.typeName || '未選択')}</div></div>${row?.reason?`<div class="hte-confirm-row"><div class="hte-confirm-label">選定理由</div><div>${esc(row.reason)}</div></div>`:''}</div><div class="hte-note" style="margin-top:10px">内容を確認し、「型候補一覧へ」を押してください。</div></div>`; }
  ...
  function render() {
    ...
    modal.innerHTML = `<div class="hte-head"><div><h2>型編成ナビ</h2><div class="hte-sub">${esc(displayVersion())} / 型タグ・目的・候補表示</div></div><button class="hte-btn" data-action="close">閉じる</button></div><div class="hte-tabs"><button class="hte-tab ${state.mode === 'main' ? 'active' : ''}" data-mode="main">主将から考える</button><button class="hte-tab ${state.mode === 'purpose' ? 'active' : ''}" data-mode="purpose">目的から考える</button><button class="hte-tab ${state.mode === 'type' ? 'active' : ''}" data-mode="type">型を直接選ぶ</button></div>${renderStepProgress()}<div class="hte-body">${renderStepBody()}</div><div class="hte-foot"><div><div class="hte-summary">主将: ${esc(state.mainGeneral?.displayName || state.mainGeneral?.name || '未選択')} / 目的: ${esc(purpose()?.purposeName || '未選択')} / 型: ${esc(typeRule()?.typeName || '未選択')}</div><div id="hadoTypeEntryMessage" class="hte-note"></div></div><div class="hte-foot-actions"><button class="hte-btn" data-action="clear">リセット</button>${state.stepIndex > 0 ? '<button class="hte-btn" data-action="back">戻る</button>' : ''}${step === 'confirm' ? '<button class="hte-btn primary" data-action="save">型候補一覧へ</button>' : `<button class="hte-btn primary" data-action="next" ${canAdvance() ? '' : 'disabled'}>次へ</button>`}</div></div>`;
