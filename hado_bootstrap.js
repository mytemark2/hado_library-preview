...
function validateJsonIndexContractDiagnostic(data,options={}){
  ...
  const skillRows=unwrapDerivedJsonItems(data?.skillOwnerIndex||{});const invalidSkillRows=skillRows.filter(row=>norm(row?.skillDomain||'')!=='generalSkill'||!norm(row?.skillKey||''));
  if(invalidSkillRows.length)problems.push(`hadou_skill_owner_index.json: generalSkill契約違反 ${invalidSkillRows.length}件`);
