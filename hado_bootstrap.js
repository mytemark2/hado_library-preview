...
let cursor=0;const worker=async()=>{while(cursor<queue.length){const row=queue[cursor++];const {key,file}=row;try{const text=await loadJsonTextByXhr(requestUrl(file));out[key]=await parseJsonText(text,file);}catch(err){if(!row.optional)throw err;out[key]=emptyOptionalJsonBundle(key);debugLog('optionalJson:missing-http',{key,file,message:err?.message||String(err)});}markLoaded(file);await nextFrame();}};
...
const REGRESSION_SELF_CHECK_SPEC={
  ...
  requiredFunctions:[
    ...
    'renderTagSearchControls','renderTagCandidates','addSelectedTag','commitTagSearchInput','clearSelectedTags','validateTypeSearchFoundationDiagnostic','validateTypeSearchReleaseReadinessDiagnostic',
