...
applyLoadedData=async function(data){
  ...
  debugLog('clauseSurfaceBridge:runtime-ready',state.diagnostics.clauseSurfaceBridge);
...
async function setCachedJsonBundle(data,source='manual',meta={}){const savedAt=new Date().toISOString();const fileManifest=meta&&meta.fileManifest&&typeof meta.fileManifest==='object'?meta.fileManifest:null;const bundleManifest=meta&&meta.bundleManifest&&typeof meta.bundleManifest==='object'?meta.bundleManifest:null;const bundleId=norm(bundleManifest?.bundleId||meta?.bundleId||'');const bundleFingerprint=fileManifest?buildJsonManifestFingerprint(fileManifest):'';const summary=extractBundleGenerationSummary(data);await jsonCacheSet(JSON_CACHE_KEY,{savedAt,source,data,fileManifest,bundleManifest,bundleId,bundleFingerprint,sourceDataSetId:summary.dataSetId,sourceCrawlerVersion:summary.crawlerVersion});debugLog('jsonCache:set',{savedAt,source,bundleId,bundleFingerprint,fileCount:fileManifest?Object.keys(fileManifest).length:0,sourceDataSetId:summary.dataSetId,sourceCrawlerVersion:summary.crawlerVersion});}
...
function validateJsonIndexContractDiagnostic(data,options={}){
  const problems=[];const requiredContractVersion='1.0';
  const feature=data?.typeSearchFeatureIndex||{};const featureItems=unwrapDerivedJsonItems(feature);
  const statusMetaItems=unwrapDerivedJsonItems(data?.statusEffectMetaIndex||{});const statusKeys=new Set(statusMetaItems.map(v=>norm(v?.statusEffectKey||'')).filter(Boolean));
  const featureRows=featureItems.flatMap(item=>[...(item?.typeFeatures||[]),...(item?.statusEffectRefs||[])]);
  if(norm(feature?.contractVersion||'')!==requiredContractVersion)problems.push(`hadou_type_search_feature_index.json: contractVersion ${feature?.contractVersion||'未設定'}`);
  const missingFeatureContract=featureRows.filter(row=>!norm(row?.sourceEntityKey||'')||!norm(row?.sourcePartType||'')||!norm(row?.canonicalFeatureKey||'')||!norm(row?.featureDomain||'')||!row?.roleGate);
  if(missingFeatureContract.length)problems.push(`hadou_type_search_feature_index.json: 契約フィールド不足 ${missingFeatureContract.length}件`);
  const articleScoreRows=featureRows.filter(row=>norm(row?.sourcePartType||'')==='article_text'&&row?.roleGate?.scoreEligible!==false);
  if(articleScoreRows.length)problems.push(`hadou_type_search_feature_index.json: article_text加点許可 ${articleScoreRows.length}件`);
  const unresolvedStatuses=featureItems.flatMap(item=>item?.statusEffectRefs||[]).filter(row=>!statusKeys.has(norm(row?.statusEffectKey||'')));
  if(unresolvedStatuses.length)problems.push(`hadou_type_search_feature_index.json: 未解決statusEffectKey ${unresolvedStatuses.length}件`);
  const skillRows=unwrapDerivedJsonItems(data?.skillOwnerIndex||{});const allowedSkillDomains=new Set(['generalSkill','advisorSkill','equipmentSkill','ethnicResearchSkill','fiveElementSkill','warhorseSkill','derivedSkill']);const invalidSkillRows=skillRows.filter(row=>!allowedSkillDomains.has(norm(row?.skillDomain||''))||!norm(row?.skillKey||'')||!Array.isArray(row?.skillDomains));
  if(invalidSkillRows.length)problems.push(`hadou_skill_owner_index.json: アプリ技能ドメイン契約違反 ${invalidSkillRows.length}件`);
  const relatedCoverage=(data?.relatedLinkIndex?.qualityAudit||getDerivedJsonMeta(data?.relatedLinkIndex||{}).qualityAudit||{}).coverageAudit||{};
  if(relatedCoverage.ok!==true||Number(relatedCoverage.missingApplicationSkillCount||0)!==0||Number(relatedCoverage.applicationSkillIndexedCount||0)!==skillRows.length)problems.push('hadou_related_link_index.json: アプリ表示技能の索引網羅監査NG');
  const relatedRows=unwrapDerivedJsonItems(data?.relatedLinkIndex||{});const invalidRelatedRefs=relatedRows.flatMap(row=>Object.entries(row?.related||{}).filter(([key,value])=>key!=='sourceRefs'&&Array.isArray(value)).flatMap(([,value])=>value)).filter(ref=>!norm(ref?.targetDomain||'')||!norm(ref?.targetKey||'')||!norm(ref?.relatedRef||''));
  if(invalidRelatedRefs.length)problems.push(`hadou_related_link_index.json: relatedRef契約違反 ${invalidRelatedRefs.length}件`);
  const parameterRows=unwrapDerivedJsonItems(data?.parameterSummaryIndex||{}).flatMap(item=>item?.effects||[]);const invalidParameters=parameterRows.filter(row=>!norm(row?.parameterKey||'')||!Array.isArray(row?.keywords)||row.keywords.some(v=>/"\]\s*,\s*\["|\\?"\s*,\s*\\?"/.test(String(v||''))));
  if(invalidParameters.length)problems.push(`hadou_parameter_summary_index.json: parameterKey/keywords契約違反 ${invalidParameters.length}件`);
  const tacticRows=unwrapDerivedJsonItems(data?.tacticAttackIndex||{});const invalidTactics=tacticRows.filter(row=>typeof row?.hasDirectDamage!=='boolean'||typeof row?.hasAnyAttackEffect!=='boolean'||typeof row?.hasConditionalAttack!=='boolean'||typeof row?.hasCounterAttack!=='boolean'||((row?.attackBlocks||[]).length>0&&row.hasAnyAttackEffect!==true));
  if(invalidTactics.length)problems.push(`hadou_tactic_attack_index.json: 攻撃種別契約違反 ${invalidTactics.length}件`);
  const focus=statusMetaItems.filter(v=>['穿撃','轟炎'].includes(norm(v?.name||'')));if(focus.length!==2||focus.some(v=>norm(v?.gameType||'')!=='有利変化'||norm(v?.actorPolarity||'')!=='selfPositive'))problems.push('hadou_status_effect_meta_index.json: 穿撃/轟炎 gameType・actorPolarity不整合');
  const result={timestamp:debugTimestamp(),context:norm(options?.context||'runtime'),ok:problems.length===0,contractVersion:feature?.contractVersion||'',featureRowCount:featureRows.length,statusKeyCount:statusKeys.size,skillOwnerCount:skillRows.length,relatedItemCount:relatedRows.length,parameterEffectCount:parameterRows.length,tacticCount:tacticRows.length,problems,policy:'JSON index contract v1: 検索語・表示名・内部キー・評価根拠を分離し、sourcePartType / roleGate / canonicalFeatureKeyを起動時に検証する。'};
  state.diagnostics.jsonIndexContract=result;debugStartup('json index contract',result);debugLog('jsonIndexContract',result);if(options?.throwOnError&&!result.ok)throw new Error('JSONインデックス契約NGです。\n- '+problems.join('\n- '));return result;
}

function validateExternalJsonBundle(data,options={}){if(!data||typeof data!=='object')throw new Error('JSONデータが不正です');for(const key of Object.keys(EXTERNAL_JSON_FILES)){if(!(key in data))throw new Error(`必要なJSONファイルが不足しています: ${EXTERNAL_JSON_FILES[key]||key}。最新クローラーで生成したJSON一式を選択してください。`);const raw=data[key];if(key==='meta'){if(!raw||typeof raw!=='object'||Array.isArray(raw))throw new Error('JSONデータ meta はオブジェクト形式である必要があります');continue;}if(!Array.isArray(raw)&&!(raw&&Array.isArray(raw.items)))throw new Error(`JSONデータ ${key} は配列または items 配列を持つ形式である必要があります`);}if(options.skipHeavy){state.diagnostics.latestJsonRequiredGate={timestamp:debugTimestamp(),structuralOnly:true,policy:'公開版はCI検証済みJSONを使用し、起動時は必須ファイルと形式のみ検査する。'};return true;}const canonical=validateRequiredDerivedJsonBundle(data);const typeSearchFoundation=validateTypeSearchFoundationDiagnostic(data);const typeSearchReleaseReadiness=validateTypeSearchReleaseReadinessDiagnostic(data,{context:'json-load',throwOnError:true});const jsonIndexContract=validateJsonIndexContractDiagnostic(data,{context:'json-load',throwOnError:true});state.diagnostics.latestJsonRequiredGate={timestamp:debugTimestamp(),...canonical,typeSearchFoundation,typeSearchReleaseReadiness,jsonIndexContract,policy:'HADO-2.9.5.0 + JSON index contract v1: 必要JSON・派生JSONメタ・型検索基盤・契約監査が揃った場合だけ起動する。'};return true;}
async function loadJsonTextByFetch(url,options={}){const timeoutMs=Number(options.timeoutMs)||WEB_JSON_CACHE_POLICY?.FILE_TIMEOUT_MS||8000;const controller=typeof AbortController==='function'?new AbortController():null;const timer=controller?setTimeout(()=>controller.abort(),timeoutMs):null;try{const res=await fetch(url,{cache:'no-store',signal:controller?.signal});if(!res.ok)throw new Error(`HTTP ${res.status} ${url}`);return await res.text();}catch(err){if(err?.name==='AbortError')throw new Error(`TIMEOUT ${timeoutMs}ms ${url}`);throw err;}finally{if(timer)clearTimeout(timer);}}
async function loadJsonTextByXhr(url,options={}){const timeoutMs=Number(options.timeoutMs)||WEB_JSON_CACHE_POLICY?.FILE_TIMEOUT_MS||8000;return await new Promise((resolve,reject)=>{try{const xhr=new XMLHttpRequest();xhr.open('GET',url,true);xhr.overrideMimeType('application/json');xhr.timeout=timeoutMs;xhr.onreadystatechange=()=>{if(xhr.readyState!==4)return;if((xhr.status>=200&&xhr.status<300)||xhr.status===0){resolve(xhr.responseText);}else{reject(new Error(`HTTP ${xhr.status} ${url}`));}};xhr.onerror=()=>reject(new Error(`XHR failed ${url}`));xhr.ontimeout=()=>reject(new Error(`TIMEOUT ${timeoutMs}ms ${url}`));xhr.send();}catch(err){reject(err);}});}
...
async function sha256JsonText(text){if(!globalThis.crypto?.subtle)throw new Error('SHA-256検証を利用できません');const bytes=new TextEncoder().encode(String(text).replace(/\r\n/g,'\n'));const digest=await crypto.subtle.digest('SHA-256',bytes);return {size:bytes.byteLength,sha256:Array.from(new Uint8Array(digest),value=>value.toString(16).padStart(2,'0')).join('')};}
async function loadWebBundleManifest(){if(!WEB_JSON_CACHE_POLICY)throw new Error('公開JSONキャッシュ機能を読み込めません');const file=WEB_JSON_CACHE_POLICY.MANIFEST_FILE;const url=`${file}?cb=${encodeURIComponent(Date.now().toString(36))}`;const text=await loadJsonTextByXhr(url,{timeoutMs:WEB_JSON_CACHE_POLICY.UPDATE_CHECK_TIMEOUT_MS});return WEB_JSON_CACHE_POLICY.normalizeManifest(await parseJsonText(text,file));}
async function loadExternalJsonBundleViaHttp(remoteManifest){
...
// XHR is used here for bounded loading, but unlike fetch(cache:'no-store') it can
// reuse a stale browser response for an unchanged JSON URL.  Give every complete
// bundle load one token so all files come from the same fresh request cycle.
const requestToken=[norm(window.HADO_APP_DISPLAY_VERSION||window.HADO_VERSION?.displayVersion||''),Date.now().toString(36)].filter(Boolean).join('-');
const requestUrl=file=>`${file}${String(file).includes('?')?'&':'?'}v=${encodeURIComponent(requestToken)}`;
...
let cursor=0;const worker=async()=>{while(cursor<queue.length){const row=queue[cursor++];const {key,file}=row;try{const text=await loadJsonTextByXhr(requestUrl(file),{timeoutMs:WEB_JSON_CACHE_POLICY?.FILE_TIMEOUT_MS||8000});const expected=remoteManifest?.files?.[file]||null;if(expected){const actual=await sha256JsonText(text);if(actual.size!==Number(expected.size)||actual.sha256!==norm(expected.sha256).toLowerCase()){debugLog('webJson:sha256-mismatch',{file,expectedSize:Number(expected.size),actualSize:actual.size,expectedSha256:expected.sha256,actualSha256:actual.sha256});throw new Error(`${file} の完全性検証に失敗しました`);}}out[key]=await parseJsonText(text,file);}catch(err){if(!row.optional||remoteManifest?.files?.[file])throw err;out[key]=emptyOptionalJsonBundle(key);debugLog('optionalJson:missing-http',{key,file,message:err?.message||String(err)});}markLoaded(file);await nextFrame();}};
await Promise.all(Array.from({length:Math.min(3,queue.length)},()=>worker()));
debugLog('webJson:parallel-load',{total,completed,requestToken,elapsedMs:Math.round(performance.now()-startedAt)});
return out;}
...
if(IS_WEB_DEPLOYMENT){
  if(!WEB_JSON_CACHE_POLICY)throw new Error('公開JSONキャッシュ機能を読み込めません');
  const cached=await getCachedJsonBundle();let cacheValid=false;let cacheError='';
  if(cached?.data){try{validateExternalJsonBundle(cached.data,{skipHeavy:true});cacheValid=true;}catch(err){cacheError=err?.message||String(err);debugLog('webJson:cache-invalid',{message:cacheError});}}
  const forceRefresh=WEB_JSON_CACHE_POLICY.consumeForceRefresh();
  const useCached=(loadDecision,reason,error='')=>{window.HADO_WEB_JSON_STARTUP_STATUS={mode:'cache-fallback',loadDecision,reason,error,bundleId:norm(cached?.bundleId||''),cacheSavedAt:cached?.savedAt||''};recordStartupDataDecision({loadDecision,reason,startupDataSource:'indexeddb-cache',cacheSavedAt:cached?.savedAt||'',cacheSource:cached?.source||'',bundleId:norm(cached?.bundleId||''),error});debugStartup('external bundle source',{source:'indexeddb-cache',reason,bundleId:norm(cached?.bundleId||'')});return cached.data;};
  if(cacheValid&&navigator.onLine===false)return useCached('auto-cache-update-check-failed','offline','navigator.onLine=false');
  setLoadingState(true,{title:'データの更新を確認しています…',detail:'公開データの更新情報を確認しています。',current:0,total:1});
  let remoteManifest;
  try{remoteManifest=await loadWebBundleManifest();}
  catch(err){if(cacheValid)return useCached('auto-cache-update-check-failed','manifest-timeout-or-failed',err?.message||String(err));throw err;}
  const cachedMeta=cached?.data?.meta||{};const cachedGenerationMatches=cacheValid&&norm(remoteManifest.dataRunId)&&norm(cachedMeta.runId)===norm(remoteManifest.dataRunId)&&norm(cachedMeta.dataUpdatedAt||cachedMeta.exportedAt)===norm(remoteManifest.dataUpdatedAt);
  if(cacheValid&&!forceRefresh&&(norm(cached.bundleId)===remoteManifest.bundleId||(!norm(cached.bundleId)&&cachedGenerationMatches))){const matchReason=norm(cached.bundleId)?'bundle-id-match':'legacy-cache-generation-match';window.HADO_WEB_JSON_STARTUP_STATUS={mode:'cache-current',loadDecision:'auto-cache-current',bundleId:remoteManifest.bundleId,cacheSavedAt:cached.savedAt||''};if(!norm(cached.bundleId))setCachedJsonBundle(cached.data,cached.source||'http',{bundleManifest:remoteManifest,fileManifest:cached.fileManifest||null}).then(()=>debugLog('webJson:legacy-cache-upgraded',{bundleId:remoteManifest.bundleId})).catch(err=>debugLog('webJson:legacy-cache-upgrade-error',{message:err?.message||String(err)}));recordStartupDataDecision({loadDecision:'auto-cache-current',reason:matchReason,startupDataSource:'indexeddb-cache',cacheSavedAt:cached.savedAt||'',cacheSource:cached.source||'',bundleId:remoteManifest.bundleId});debugStartup('external bundle source',{source:'indexeddb-cache',reason:matchReason,bundleId:remoteManifest.bundleId});return cached.data;}
  try{
    const data=await loadExternalJsonBundleViaHttp(remoteManifest);
    validateExternalJsonBundle(data,{skipHeavy:true});
    try{await setCachedJsonBundle(data,'http',{bundleManifest:remoteManifest});debugLog('webJson:cache-save-complete',{source:'http',bundleId:remoteManifest.bundleId});}
    catch(cacheSaveErr){debugLog('webJson:cache-save-error',{message:cacheSaveErr?.message||String(cacheSaveErr),bundleId:remoteManifest.bundleId});}
    window.HADO_WEB_JSON_STARTUP_STATUS={mode:'server-updated',loadDecision:'auto-http-update',bundleId:remoteManifest.bundleId,previousBundleId:norm(cached?.bundleId||'')};
    recordStartupDataDecision({loadDecision:'auto-http-update',reason:forceRefresh?'manual-force-refresh':(cacheValid?'bundle-id-changed':'no-valid-cache'),startupDataSource:'http',directoryHandleAvailable:false,directoryPermission:'not-applicable',bundleId:remoteManifest.bundleId,previousBundleId:norm(cached?.bundleId||''),cacheValidationError:cacheError});
    debugStartup('external bundle source',{source:'http',cacheSave:'complete-or-nonblocking-failure',bundleId:remoteManifest.bundleId});
    return data;
  }catch(err){if(cacheValid)return useCached('auto-cache-refresh-failed','bundle-refresh-timeout-or-failed',err?.message||String(err));throw err;}
}
...
const REGRESSION_SELF_CHECK_SPEC={
  ...
  requiredFunctions:[
    ...
    'renderTagSearchControls','renderTagCandidates','addSelectedTag','selectTagCandidate','clearSelectedTags','validateTypeSearchFoundationDiagnostic','validateTypeSearchReleaseReadinessDiagnostic',
...
// HADO-3.0.0.0-IME-SEARCH-COMMIT-START
function isSearchImeCompositionEvent(event){
  return !!(state._searchComposing||event?.isComposing||Number(event?.keyCode)===229);
}
function setupSearchImeCommitGuard(){
  const input=els.searchInput;
  if(!input||state._searchImeCommitGuardInstalled)return;
  state._searchImeCommitGuardInstalled=true;
  state._searchImeCommitPending=false;
  state._searchImeCommitValue='';
  input.addEventListener('compositionstart',event=>{
    cancelScheduledSearchRender();
    state._searchComposing=true;
    state._searchImeCommitPending=false;
    state._searchImeCommitValue='';
    debugLog('search:composition-start',{});
    event.stopImmediatePropagation();
  },true);
  input.addEventListener('compositionend',event=>{
    state._searchComposing=false;
    state._searchImeCommitPending=true;
    state._searchImeCommitValue=input.value;
    clearQuickStatusEffectOwnerFilter();
    debugLog('search:composition-end',{keyword:norm(input.value)});
    scheduleSearchAndDetailRender('search-input-compositionend');
    event.stopImmediatePropagation();
  },true);
  input.addEventListener('input',event=>{
    if(state._searchComposing||event.isComposing){
      debugLog('search:debounce-skip',{reason:'composition'});
      event.stopImmediatePropagation();
      return;
    }
    if(state._searchImeCommitPending&&input.value===state._searchImeCommitValue){
      state._searchImeCommitPending=false;
      debugLog('search:debounce-skip',{reason:'composition-commit-duplicate-input',keyword:norm(input.value)});
      event.stopImmediatePropagation();
      return;
    }
    state._searchImeCommitPending=false;
...
window.HADO_TROOP_SKILLS?.install?.();
startup();
