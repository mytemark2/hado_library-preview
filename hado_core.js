...
function buildDetailConditionPresentation(item,category,sourceTexts){const api=window.HADO_DETAIL_CONDITION_PRESENTER;if(!api||typeof api.buildViewModel!=='function'||typeof api.renderHtml!=='function')return {html:'',reviewed:false,fallback:false,view:null};try{const normalizedSources=(Array.isArray(sourceTexts)?sourceTexts:[]).map(norm).filter(Boolean);if(normalizedSources.length>1)normalizedSources.push(normalizedSources.join('\n'));const options={category:category||detailCategory(item),name:getItemDisplayName(item),sourceTexts:normalizedSources};const view=api.buildViewModel(options);const html=api.renderHtml(options);return {html,reviewed:!view.empty&&!view.fallback&&view.reviewedCaseCount>0,fallback:!!view.fallback,view};}catch(err){debugLog('detail-condition:render-error',{category:category||detailCategory(item),name:getItemDisplayName(item),message:err?.message||String(err)});return {html:'',reviewed:false,fallback:false,view:null};}}
