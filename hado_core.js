...
function renderReferencedSkillCard(entry){
  ...
  return `<div class="general-skill-card referenced-skill-card" data-referenced-skill="1" data-referenced-skill-name="${esc(entry?.name||'')}"><div class="row between" style="align-items:flex-start;gap:12px"><div class="general-skill-name" style="margin-bottom:0">${esc(entry?.name||'')}</div><div><span class="badge">付与Lv: ${esc(entry?.level||'')}</span><span class="badge">技能データ参照</span></div></div>${contentHtml}</div>`;
}
...
function buildDetailConditionPresentation(item,category,sourceTexts){const api=window.HADO_DETAIL_CONDITION_PRESENTER;if(!api||typeof api.buildViewModel!=='function'||typeof api.renderHtml!=='function')return {html:'',reviewed:false,grouped:false,fallback:false,view:null};try{const normalizedSources=(Array.isArray(sourceTexts)?sourceTexts:[]).map(norm).filter(Boolean);if(normalizedSources.length>1)normalizedSources.push(normalizedSources.join('\n'));const options={category:category||detailCategory(item),name:getItemDisplayName(item),sourceTexts:normalizedSources};const view=api.buildViewModel(options);const html=api.renderHtml(options);const grouped=!view.empty&&!view.fallback&&Array.isArray(view.displaySources)&&view.displaySources.length>0;return {html,reviewed:grouped&&view.reviewedCaseCount>0,grouped,fallback:!!view.fallback,view};}catch(err){debugLog('detail-condition:render-error',{category:category||detailCategory(item),name:getItemDisplayName(item),message:err?.message||String(err)});return {html:'',reviewed:false,grouped:false,fallback:false,view:null};}}
