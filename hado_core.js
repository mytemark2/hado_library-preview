...
const DATASET_LABELS={generals:'武将',tactics:'戦法',skills:'技能',troopSkills:'兵科',equipments:'装備',statusEffects:'状態変化',siegeWeapons:'兵器',ethnicArmaments:'武装',formations:'陣形',warhorses:'名馬',warhorseSkills:'軍馬技能'};
const SEARCH_CATEGORY_DISPLAY_ORDER=Object.freeze(['generals','tactics','skills','equipments','statusEffects','siegeWeapons','ethnicArmaments','formations','warhorses','warhorseSkills','troopSkills']);
...
const FILE_META={fileName:"hado_library_3.1.2.0.html",createdAt:"2026-09-22 00:00:00"};
// FEATURE[HADO-VERSIONING]: 2.x.x.y バージョン管理。メジャー/マイナーはユーザー採番、x は機能追加、y は不具合修正。
const HADO_BUILD_INFO={version:"3.1.2.0",baseVersion:"3.1.1.0",changeType:"feature-improvement",summary:"Add searchable troop skills and remove the Clause Shadow card from the user interface.",baseSha256:"be9d13f9c3180c1de1917383c59adcb5e526214e"};
...
function renderWarhorseFormationScreen(){
  ...
  const shareActions=window.HadoShare?.renderWarhorseActionsHtml(!!currentSave,ownedList.length>0)||'';
  els.warhorseRoot.innerHTML=`<div class="warhorse-layout"><section class="warhorse-card"><div class="warhorse-card-header"><div class="warhorse-card-title"><h3>登録済み軍馬</h3><div class="note">${currentSave?`登録 ${ownedList.length}件`:'保存データ未選択'}。カードを押すと編集ダイアログを開きます。</div></div><div class="warhorse-header-actions">${shareActions}<button type="button" id="warhorseOpenCreateBtn" class="warhorse-primary-btn" ${currentSave?'':'disabled'}>＋ 軍馬を登録</button></div></div><div class="warhorse-card-body"><div class="warhorse-list">${ownedRows}</div></div></section></div>${buildWarhorseCreateDialogHtml()}${buildWarhorseEditDialogHtml(warhorseData)}${buildWarhorseDeleteConfirmBarHtml(warhorseData)}${buildWarhorseUndoToastHtml()}`;
...
function getReferencedSkillLevelContent(entry){const rawContent=(Array.isArray(entry?.content)?entry.content:[]).filter(line=>!isOwnerListContentLine(line)).join(' ');return extractRomanLevelBlockText(rawContent,entry?.level)||rawContent;}
function renderGeneralSkillLevelBadge(level,label='この武将の技能レベル'){const normalized=normalizeSkillLevelValue(level);return normalized?`<span class="general-skill-level-badge" aria-label="${esc(label)}">Lv${esc(normalized)}</span>`:'';}
function renderReferencedSkillCard(entry){
  ...
  return `<div class="general-skill-card referenced-skill-card" data-referenced-skill="1" data-referenced-skill-name="${esc(entry?.name||'')}"><div class="general-skill-heading"><div class="general-skill-name" style="margin-bottom:0">${esc(entry?.name||'')}</div>${renderGeneralSkillLevelBadge(entry?.level||'','付与される技能レベル')}</div>${levelHtml||fallbackHtml}</div>`;
}
...
function buildDetailConditionPresentation(item,category,sourceTexts){const bridge=window.HADO_CLAUSE_SURFACE_BRIDGE;const presenter=window.HADO_DETAIL_CONDITION_PRESENTER;if(!bridge&&!presenter)return {html:'',reviewed:false,grouped:false,fallback:false,view:null,projection:null};try{const normalizedSources=(Array.isArray(sourceTexts)?sourceTexts:[]).map(norm).filter(Boolean);if(normalizedSources.length>1)normalizedSources.push(normalizedSources.join('\n'));const options={category:category||detailCategory(item),name:getItemDisplayName(item),sourceTexts:normalizedSources};const result=bridge&&typeof bridge.buildDetailPresentation==='function'?bridge.buildDetailPresentation(options):{view:presenter.buildViewModel(options),html:presenter.renderHtml(options),projection:null};const view=result.view;const grouped=!view.empty&&!view.fallback&&Array.isArray(view.displaySources)&&view.displaySources.length>0;return {html:result.html,reviewed:grouped&&view.reviewedCaseCount>0,grouped,fallback:!!view.fallback,view,projection:result.projection||null};}catch(err){debugLog('detail-condition:render-error',{category:category||detailCategory(item),name:getItemDisplayName(item),message:err?.message||String(err)});return {html:'',reviewed:false,grouped:false,fallback:false,view:null,projection:null};}}
function renderSectionsHtml(sections){const fs=filterSections(sections).filter(sec=>norm(sec?.title||'')!=='侍従の配置可能位置と条件');if(!fs.length)return '';return `<table class="section-table">${fs.map(sec=>{const title=norm(sec?.title||'');const content=Array.isArray(sec?.content)?sec.content:[];if(isMergedTitle(title))return `<tr><td colspan="2" class="merge-cell no-detail-linkify">${escDisplay(title)}</td></tr>`;return `<tr><td class="detail-structure-label no-detail-linkify">${escDisplay(title)}</td><td>${fmtContent(content)}</td></tr>`;}).join('')}</table>`;}
function renderRaw(item){return `<pre class="raw">${esc(JSON.stringify(item?.raw||item||{},null,2))}</pre>`;}
function renderEquipmentKeyValueRows(rows){if(!Array.isArray(rows)||!rows.length)return '';return `<table class="kv-table">${rows.map(row=>{const key=Array.isArray(row)?row[0]:'';const value=Array.isArray(row)?row[1]:'';const keyText=norm(key);const valueHtml=keyText==='侍従位置'?renderJijuPositionDisplayHtml(value):(keyText==='相性'?renderAffinityClockHtml(value):escDisplay(value));return `<tr><td class="detail-structure-label no-detail-linkify" style="width:220px">${escDisplay(key)}</td><td>${valueHtml}</td></tr>`;}).join('')}</table>`;}
...
function renderGeneralTactic(item){const table=findTacticTable(item);if(!Array.isArray(table)||table.length<6)return '';const valueRow=Array.isArray(table[1])?table[1]:[];const effectLine=norm((table[3]||[])[0]||'');const timingHeaders=Array.isArray(table[4])?table[4]:[];const timingValues=Array.isArray(table[5])?table[5]:[];const summaryRows=[['戦法名',findTacticName(item)],['兵科',valueRow[0]||''],['効果系統',valueRow[1]||''],['攻撃属性',valueRow[2]||''],['発動間隔',timingValues[0]||''],['対象範囲',timingValues[1]||''],['発動率',timingValues[2]||''],['連鎖順',timingValues[3]||'']];const condition=buildDetailConditionPresentation(item,'generals',[effectLine]);const rawHtml=`<div class="general-text">${fmtTacticLinkedContent([effectLine])}</div>`;const effectHtml=condition.reviewed?condition.html:[condition.html,rawHtml].filter(Boolean).join('');return `<div class="general-card"><div class="general-card-header">戦法</div><div class="general-card-body"><div class="general-two-col"><div>${renderEquipmentKeyValueRows(summaryRows)}</div><div class="general-card" style="box-shadow:none"><div class="general-card-header">効果概要</div><div class="general-card-body">${effectHtml}</div></div></div></div></div></div>`;}
...
function renderSkillDetail(item){
  ...
  const sourceTexts=[...(Array.isArray(item?.sections)?item.sections:[]).flatMap(sec=>Array.isArray(sec?.content)?sec.content:[]),...(Array.isArray(item?.raw?.sections)?item.raw.sections:[]).flatMap(sec=>Array.isArray(sec?.content)?sec.content:[])];
  const condition=buildDetailConditionPresentation(item,'skills',sourceTexts);
  const html=[condition.html,levelHtml,baseHtml,stageHtml].filter(Boolean).join('');
