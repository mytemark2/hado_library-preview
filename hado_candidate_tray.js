...
const DRAFT_SNAPSHOT='hado:type-candidate-draft-snapshot';
const DRAFT_SNAPSHOT_REQUEST='hado:type-candidate-draft-snapshot-request';
let snapshot={formationId:'',formationName:'',evaluationTypeId:'',items:[]};
let draftSnapshot={active:false,count:0};
...
function refreshButton(){const n=draftSnapshot.active?Number(draftSnapshot.count||0):(Array.isArray(snapshot.items)?snapshot.items.length:0);const c=document.getElementById('hct-count');if(c)c.textContent=String(n)}
...
function mount(){if(document.getElementById('hct-open'))return;style();const b=document.createElement('button');b.id='hct-open';b.type='button';b.innerHTML='候補ワークスペース <span id="hct-count">0</span>';b.onclick=()=>open().catch?.(err=>alert(`候補ワークスペースの読込に失敗しました。\n${err.message}`));document.body.appendChild(b);requestSnapshot('mount');dispatchEvent(new CustomEvent(DRAFT_SNAPSHOT_REQUEST));syncVisibility();new MutationObserver(syncVisibility).observe(document.documentElement,{attributes:true,subtree:true,attributeFilter:['class']});setInterval(syncVisibility,400)}
addEventListener(SNAPSHOT,e=>{snapshot=e.detail||{formationId:'',formationName:'',evaluationTypeId:'',items:[]};refreshButton()});
addEventListener(DRAFT_SNAPSHOT,e=>{draftSnapshot=e.detail||{active:false,count:0};refreshButton()});
