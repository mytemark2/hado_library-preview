...
function getFastSearchableText(item){
  if(!item)return '';
  if(normalizeDerivedSearchCategory(detailCategory(item))==='equipments')return buildEquipmentStageAwareSearchableText(item);
...
function resolveAvailableTagInput(value){const input=norm(value);if(!input)return '';if(state.availableTags.includes(input))return input;const q=input.toLowerCase();const exact=(state.tagCandidateIndex||[]).filter(row=>row.valueLower===q||row.displayLower===q);return exact.length===1?exact[0].tag:'';}
...
function selectTagCandidate(tag,reason='candidate-tap'){const value=norm(tag);if(!value)return false;if(els.tagSearchInput)els.tagSearchInput.value='';hideTagCandidates();const added=addSelectedTag(value,reason);if(els.tagSearchInput&&reason!=='input-exact'&&reason!=='input-compositionend')els.tagSearchInput.focus({preventScroll:true});return added;}
...
function moveTagCandidateActive(delta){const current=Number.isInteger(state.tagCandidateActiveIndex)?state.tagCandidateActiveIndex:-1;return setTagCandidateActiveIndex(current+delta);}
function commitTagSearchInput(reason='input-exact'){if(!els.tagSearchInput)return false;const value=norm(els.tagSearchInput.value),resolved=resolveAvailableTagInput(value);if(!resolved){debugLog('tagSearch:input-pending',{value,reason,candidateCount:(state.tagCandidateValues||[]).length});return false;}return selectTagCandidate(resolved,reason);}
