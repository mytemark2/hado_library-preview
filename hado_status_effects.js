...
function normalizeHadouTableRows(table,fallbackIndex=-1){const rows=Array.isArray(table)?table:(table&&typeof table==='object'&&Array.isArray(table.rows)?table.rows:[]);if(!Array.isArray(rows))return [];const sourceIndex=Number(table&&typeof table==='object'?(table._sourceIndex??table.index??fallbackIndex):fallbackIndex);if(Number.isFinite(sourceIndex))Object.defineProperty(rows,'_sourceIndex',{value:sourceIndex,enumerable:false,configurable:true});return rows;}
function normalizeHadouTables(tables){return (Array.isArray(tables)?tables:[]).map((table,index)=>normalizeHadouTableRows(table,index)).filter(table=>Array.isArray(table));}
...
const DERIVED_SEARCH_CATEGORY_ALIASES={generals:'generals',tactics:'tactics',skills:'skills',troopSkills:'troop_skills',troop_skills:'troop_skills',equipments:'equipments',statusEffects:'status_effects',status_effects:'status_effects',siegeWeapons:'siege_weapons',siege_weapons:'siege_weapons',ethnicArmaments:'ethnic_armaments',ethnic_armaments:'ethnic_armaments',ethnicResearchSkills:'ethnic_research_skills',ethnic_research_skills:'ethnic_research_skills',formations:'formations',fiveElements:'five_elements',five_elements:'five_elements',warhorses:'warhorses',warhorseSkills:'warhorse_skills',warhorse_skills:'warhorse_skills'};
// HADO-3.0.0.0-SEARCH-ROSTER-EXCLUSION: 他武将一覧は本人の説明ではないため全文検索から除外する。
// HADO-3.0.0.0-GENERAL-SEARCH-SOURCE-BOUNDARY: 基本情報と戦法の間は攻略評価・おすすめ・比較コメントであり、効果本文ではない。
function isGeneralCommentarySearchSection(item,section){const sections=Array.isArray(item?.sections)?item.sections:[];if(!sections.length||!section)return false;let sectionIndex=sections.indexOf(section);if(sectionIndex<0){const title=norm(section?.title||'');const content=JSON.stringify(Array.isArray(section?.content)?section.content:[]);sectionIndex=sections.findIndex(candidate=>norm(candidate?.title||'')===title&&JSON.stringify(Array.isArray(candidate?.content)?candidate.content:[])===content);}if(sectionIndex<0)return false;const basicIndex=sections.findIndex(candidate=>/の基本情報$/.test(norm(candidate?.title||'')));const tacticIndex=sections.findIndex(candidate=>/の戦法$/.test(norm(candidate?.title||'')));const commentaryStart=basicIndex>=0?basicIndex+1:1;return tacticIndex>commentaryStart&&sectionIndex>=commentaryStart&&sectionIndex<tacticIndex;}
function isSearchExcludedSection(item,section){const title=norm(section?.title||'');const category=norm(item?.sourceDataset||item?.category||'');if(!title)return false;if(/の入手方法$/.test(title))return true;if(category==='generals'&&(isGeneralCommentarySearchSection(item,section)||/列伝$/.test(title)||/^(演義|正史|兵科の基本能力|各レベルの能力|兵科ランク上昇タイミング|解説)$/.test(title)||/の兵科$/.test(title)||/と相性の良い武将$/.test(title)||/^相性の良い(?:LR|UR|SSR|SR|N・R)?武将$/.test(title)||/の専用名宝$/.test(title)||/の五行適正$/.test(title)||title==='五行適正'))return true;if(category==='equipments')return true;return false;}
function isSearchExcludedTable(item,table){const category=norm(item?.sourceDataset||item?.category||'');if(!table)return false;const rows=Array.isArray(table)?table:(Array.isArray(table?.rows)?table.rows:[]);if(!rows.length)return false;const sourceIndex=Number(table?._sourceIndex??table?.index);const cells=rows.flatMap(row=>Array.isArray(row)?row:[]).map(value=>norm(value)).filter(Boolean);const joined=cells.join(' ');const troopBase=norm(rows?.[0]?.[0]||'')==='兵科'&&/輸送|機動|射程|攻撃間隔/.test(joined)&&!/効果系統|対象範囲|発動率/.test(joined);const troopLevel=cells.includes('解放将星')&&cells.includes('基礎兵力');if(category==='generals'&&([20,21].includes(sourceIndex)||troopBase||troopLevel))return true;const rosterIconCount=cells.reduce((count,value)=>count+((value.match(/アイコン/g)||[]).length),0);if(category==='generals'&&rosterIconCount>=4&&cells.some(value=>/(?:LR|UR|覚醒|武将)/.test(value)))return true;const firstRow=Array.isArray(rows[0])?rows[0]:[];const first0=norm(firstRow[0]||'');const first1=norm(firstRow[1]||'');if(category==='equipments'&&first0==='武将')return true;if(category==='generals'&&first0==='名宝'&&first1==='技能効果')return true;return false;}
function isSearchExcludedContentLine(item,section,line){const category=norm(item?.sourceDataset||item?.category||'');const text=norm(line||'');if(category!=='generals'||!text)return false;return /を持つ武将/.test(text)||/を強化する装備品/.test(text)||/武将一覧はこちら/.test(text);}
function sanitizeSectionForSearch(item,section){if(!section||isSearchExcludedSection(item,section))return null;return {...section,content:(Array.isArray(section.content)?section.content:[]).filter(line=>!isSearchExcludedContentLine(item,section,line))};}
function sanitizeRawForSearch(item,raw){if(!raw||typeof raw!=='object')return raw;const sanitized={...raw};delete sanitized.textSample;if(Array.isArray(raw.sections))sanitized.sections=raw.sections.map(sec=>sanitizeSectionForSearch(item,sec)).filter(Boolean);if(Array.isArray(raw.tables))sanitized.tables=raw.tables.filter(tbl=>!isSearchExcludedTable(item,tbl));return JSON.parse(JSON.stringify(sanitized));}
function buildSearchableText(item){const base=[item?.name,item?.title,item?.description,item?.sourceName,item?.sourceGeneral,item?.raw?.sourceGeneral,...(Array.isArray(item?.sourceNames)?item.sourceNames:[]),...(Array.isArray(item?.searchTokens)?item.searchTokens:[])].filter(Boolean).map(v=>norm(v)).join(' ');const sections=(Array.isArray(item?.sections)?item.sections:[]).map(sec=>sanitizeSectionForSearch(item,sec)).filter(Boolean).map(sec=>[sec?.title,...(Array.isArray(sec?.content)?sec.content:[])].filter(Boolean).join(' ')).join(' ');const tables=(Array.isArray(item?.tables)?item.tables:[]).filter(tbl=>!isSearchExcludedTable(item,tbl)).map(tbl=>stringifyWithoutTextSample(tbl)).join(' ');const rawForSearch=sanitizeRawForSearch(item,item?.raw);const rawText=typeof rawForSearch==='object'?stringifyWithoutTextSample(rawForSearch):String(rawForSearch||'');const tacticAttackText=detailCategory(item)==='tactics'||detailCategory(item)==='generals'?buildTacticAttackSearchText(item):'';return norm([base,sections,tables,rawText,tacticAttackText].filter(Boolean).join(' ')).toLowerCase();}
...
function getResultCardBadgesHtml(row){const card=getResultCardIndexForRow(row);const badges=Array.isArray(card?.badges)?card.badges.map(norm).filter(Boolean).filter(isAllowedSearchResultBadge).slice(0,4):[];return badges.map(v=>`<span class="search-result-mini-badge is-dataset">${esc(v)}</span>`).join('');}
...
function buildDerivedTagIndexLookup(){
  const bucket=state?.derivedData?.tagIndex;
  const items=bucket&&bucket.available&&Array.isArray(bucket.items)?bucket.items:[];
  const byName=new Map(),byKey={},byKeyCategories={},tagSet=new Set(),blockedTags=new Set();
  items.forEach(entry=>{
    const cat=normalizeDerivedSearchCategory(entry?.category||'');
    const tags=(Array.isArray(entry?.tags)?entry.tags:[]).map(norm).filter(Boolean).filter(tag=>{if(isAllowedSearchTag(tag))return true;blockedTags.add(tag);return false;});
    if(!cat||!tags.length)return;
    [entry?.name,entry?.rawName,entry?.title,entry?.displayName].map(norm).filter(Boolean).forEach(name=>{const key=makeDerivedSearchIndexKey(cat,name);if(!byName.has(key))byName.set(key,tags);});
    tags.forEach(tag=>{
      tagSet.add(tag);
      const p=tag.indexOf(':'),key=p>=0?tag.slice(0,p):'その他';
      if(!byKey[key])byKey[key]=new Set();
      if(!byKeyCategories[key])byKeyCategories[key]=new Set();
      byKey[key].add(tag);
      byKeyCategories[key].add(cat);
    });
  });
...
function applyDerivedTagIndexToItems(allItems){
  ...
  allItems.forEach(item=>{
    ...
    if(!diag.byCategory[cat])diag.byCategory[cat]={total:0,applied:0,fallback:0};
    diag.byCategory[cat].total++;
  ...
  state.availableTagGroupOrder=[...lookup.tagGroupOrder];
  state.selectedTags=(state.selectedTags||[]).filter(tag=>lookup.tagSet.has(tag));
  diag.tagGroups=state.availableTagGroupOrder.map(key=>({key,categories:state.availableTagCategoriesByKey[key]||[]}));
  state.diagnostics.tagIndex=diag;
  debugLog('tagSearch:derived-index-built',diag);
  return true;
}
...
function buildEquipmentStageAwareSearchableText(item){
  if(!item)return '';
  const stage=getEffectiveEquipmentStageForItem(item);
  const savedKey=state.viewMode==='saved'?(state.savedSearchCacheKey||buildSavedSearchCacheKey(getCurrentSave())):'';
  const cacheKey=`${state.viewMode||'all'}|${stage}|${savedKey}`;
  if(item._equipmentStageSearchableTextCache?.key===cacheKey)return item._equipmentStageSearchableTextCache.text||'';
  const entry=item._derivedSearchIndexEntry||null;
  const basic=[getItemDisplayName(item),item?.name,item?.rawName,item?.title];
  ['最高レア','種類','兵科','レア'].forEach(label=>{const value=findFirstTableValue(item,label);if(norm(value))basic.push(label,value,`${label}${value}`);});
  const base=norm(entry?.baseSearchText||basic.join(' ')).toLowerCase();
  let stageText=norm(entry?.equipmentStageSearchText?.[stage]||'').toLowerCase();
  if(!stageText){
    const selected=selectEquipmentSkillStageBlock(item,[],stage);
    const block=selected?.block||null;
    const rows=Array.isArray(block?.table)?block.table:[];
    stageText=norm([selected?.requestedLabel||equipmentStageLabel(stage),...rows.flatMap(row=>Array.isArray(row)?row:[row])].join(' ')).toLowerCase();
  }
  const text=norm([base,equipmentStageLabel(stage),stageText].filter(Boolean).join(' ')).toLowerCase();
  item._equipmentStageSearchableTextCache={key:cacheKey,text};
  return text;
}
function getFastSearchableText(item){
  if(!item)return '';
  const rootApi=(typeof window!=='undefined'?window:globalThis);const clauseApi=rootApi.HADO_CLAUSE_SURFACE_BRIDGE||rootApi.HADO_SEARCH_CLAUSE_INTEGRATION;const clauseText=clauseApi&&typeof clauseApi.getEntitySearchText==='function'?clauseApi.getEntitySearchText(detailCategory(item),getItemDisplayName(item)):'';
  if(normalizeDerivedSearchCategory(detailCategory(item))==='equipments'){if(!clauseText)return buildEquipmentStageAwareSearchableText(item);const equipmentText=buildEquipmentStageAwareSearchableText(item);return `${equipmentText} ${clauseText}`;}
  ...
  const text=`${String(raw).toLowerCase()} ${clauseText}`.trim();
...
function metricMatchesSearchQuery(metric,metricSearch){
  if(!metricSearch||metricSearch.mode==='plain')return true;
  if(!metric)return false;
  const expectedSign=String(metricSearch.sign||'');
  const expectedNumber=String(metricSearch.numberText||'');
  const expectedUnit=String(metricSearch.unit||'').replace('％','%');
  const matches=Array.isArray(metric.matches)&&metric.matches.length?metric.matches:[metric];
  return matches.some(match=>{
    const display=String(match?.display||'');
    const sign=Object.prototype.hasOwnProperty.call(match||{},'sign')?String(match?.sign||''):(display.startsWith('-')?'-':(display.startsWith('+')?'+':''));
    if(expectedSign&&sign!==expectedSign)return false;
    if(expectedNumber){const actual=Number(match?.numberText??Math.abs(Number(match?.value)));const expected=Number(expectedNumber);if(!Number.isFinite(actual)||!Number.isFinite(expected)||actual!==expected)return false;}
    if(expectedUnit&&String(match?.unit||'').replace('％','%')!==expectedUnit)return false;
    return true;
  });
}
...
function getDatasetByCategory(category){const map={generals:state.generals,tactics:state.tactics,skills:state.skills,troopSkills:state.troopSkills,equipments:state.equipments,statusEffects:state.statusEffects,siegeWeapons:state.siegeWeapons,ethnicArmaments:state.ethnicArmaments,formations:state.formationMasters,warhorses:getSearchWarhorseItems(),warhorseSkills:state.warhorseSkills};return Array.isArray(map[category])?map[category]:[];}
...
function syncResultSelectSelection(){if(!els.resultSelect)return;const opts=[...els.resultSelect.options];opts.forEach(opt=>{if(opt.value==='')opt.selected=!state.selectedItem;else{const row=state.lastResultRows[Number(opt.value)];opt.selected=!!row&&row.item===state.selectedItem;}});if(typeof renderMobileResultSummary==='function')renderMobileResultSummary('syncResultSelectSelection');}
...
function getOperationSearchModeContextSnapshot(){if(typeof captureSearchModeContext!=='function')return null;const mode=['normal','status','type'].includes(norm(state.searchMode))?norm(state.searchMode):'normal';const context=captureSearchModeContext(mode);return {initialized:true,activeCategories:safeCloneForDebug(context.activeCategories||{}),selectedTags:[...(context.selectedTags||[])],selection:context.selection?{category:context.selection.category,name:context.selection.name,label:context.selection.label,detailActiveTab:context.selection.detailActiveTab}:null,keyword:context.keyword||'',nameOnly:!!context.nameOnly,quickFilter:context.quickFilter?safeCloneForDebug(context.quickFilter):null,quickGroup:context.quickGroup||'all',quickEffect:context.quickEffect||'',typePresetId:context.typePresetId||'',typePresetDirty:!!context.typePresetDirty,typeStatusIds:[...(context.typeStatusIds||[])],typeFeatureIds:[...(context.typeFeatureIds||[])],resultRenderLimit:Number(context.resultRenderLimit)||100};}
function getOperationSnapshot(){const item=state.selectedItem;const category=item?detailCategory(item):'';return {keyword:els.searchInput?els.searchInput.value:'',selectedTags:[...(state.selectedTags||[])],searchMode:state.searchMode||'normal',searchModeContext:getOperationSearchModeContextSnapshot(),viewMode:state.viewMode||'all',typeSearchSelectedPresetId:state.typeSearchSelectedPresetId||'',typeSearchPresetDirty:!!state.typeSearchPresetDirty,typeSearchSelectedStatusEffectIds:[...(state.typeSearchSelectedStatusEffectIds||[])],typeSearchSelectedFeatureIds:[...(state.typeSearchSelectedFeatureIds||[])],selectedCategory:category,selectedName:item?getItemDisplayName(item):'',selectedLabel:state.selectedLabel||getCategoryLabel(category),activeCategories:safeCloneForDebug(state.activeCategories)};}
...
function buildResultsCopyText(){if(window.HadoShare?.buildResultsCopyText)return window.HadoShare.buildResultsCopyText();const lines=['検索条件',`検索語：${norm(currentKeyword())||'未指定'}`,'','検索結果'];(state.lastResultRows||[]).forEach(row=>lines.push(`【${row.label||'項目'}】${stripReadingForCopy(getItemDisplayName(row.item)||'-')}`));return lines.join('\n');}
...
function getSearchTagsForItem(item){const category=detailCategory(item);const tags=[];if(category==='generals')tags.push(...extractGeneralDetailTags(item));else if(category==='equipments')tags.push(...extractEquipmentDetailTags(item));else if(category==='statusEffects')tags.push(...extractStatusEffectDetailTags(item));else if(category==='skills')tags.push(...extractSkillDetailTags(item));else if(category==='siegeWeapons')tags.push(...extractSiegeWeaponDetailTags(item));else if(category==='ethnicArmaments')tags.push(...extractEthnicArmamentDetailTags(item));const rootApi=(typeof window!=='undefined'?window:globalThis);const clauseApi=rootApi.HADO_CLAUSE_SURFACE_BRIDGE||rootApi.HADO_SEARCH_CLAUSE_INTEGRATION;if(clauseApi&&typeof clauseApi.getEntityTags==='function')tags.push(...clauseApi.getEntityTags(category,getItemDisplayName(item)));return [...new Set(tags.map(norm).filter(Boolean).filter(isAllowedSearchTag))];}
function buildDetailTagsHtml(item){const tags=getSearchTagsForItem(item);const selected=new Set(getMatchedSelectedTags(item));debugLog('detailTags:render',{category:detailCategory(item),name:getItemDisplayName(item),tags,matchedSelectedTags:[...selected]});return `<div>${tags.map(t=>selected.has(t)?`<span class="badge tag-highlight-token ${getTagColorClass(t)}" data-selected-tag="${esc(t)}">${esc(t)}</span>`:`<span class="badge">${esc(t)}</span>`).join('')}</div>`;}
function rebuildAvailableTagCategoryIndex(allItems){const byTag={};(Array.isArray(allItems)?allItems:[]).forEach(item=>{const category=detailCategory(item);if(!category)return;(item?._detailTags||getSearchTagsForItem(item)).map(norm).filter(Boolean).forEach(tag=>{if(!byTag[tag])byTag[tag]=new Set();byTag[tag].add(category);});});const ownerless=(state.availableTags||[]).filter(tag=>!byTag[tag]);const valid=new Set(Object.keys(byTag));state.availableTagCategoriesByTag=Object.fromEntries(Object.entries(byTag).map(([tag,categories])=>[tag,sortTagCategoryKeys([...categories])]));state.availableTags=(state.availableTags||[]).filter(tag=>valid.has(tag));state.availableTagsByKey=Object.fromEntries(Object.entries(state.availableTagsByKey||{}).map(([key,tags])=>[key,(tags||[]).filter(tag=>valid.has(tag))]).filter(([,tags])=>tags.length));state.selectedTags=(state.selectedTags||[]).filter(tag=>valid.has(tag));if(state.diagnostics?.tagIndex)state.diagnostics.tagIndex.ownerAudit={tagCount:valid.size,ownerlessPrunedCount:ownerless.length,ownerlessPruned:ownerless};return {tagCount:valid.size,ownerlessPrunedCount:ownerless.length};}
function buildSearchTagIndex(){
  const all=[...state.generals,...state.tactics,...state.skills,...state.equipments,...state.statusEffects,...state.siegeWeapons,...state.ethnicArmaments,...state.ethnicResearchSkills,...state.formationMasters,...state.fiveElements,...state.warhorses,...state.warhorseSkills];
  if(applyDerivedTagIndexToItems(all)){mergeClauseSearchTags(all);rebuildAvailableTagCategoryIndex(all);rebuildTagCandidateSearchIndex();debugLog('tagSearch:indexBuilt',{source:'hadou_tag_index.json+source-markers',totalTags:state.availableTags.length,keys:Object.keys(state.availableTagsByKey),sample:state.availableTags.slice(0,20),diagnostic:state.diagnostics.tagIndex});renderTagSearchControls();return;}
  const tagSet=new Set(),byKey={},byKeyCategories={};
  all.forEach(item=>{
    const category=detailCategory(item),tags=getSearchTagsForItem(item);
    item._detailTags=tags;
    tags.forEach(tag=>{
      tagSet.add(tag);
      const p=tag.indexOf(':'),key=p>=0?tag.slice(0,p):'その他';
      if(!byKey[key])byKey[key]=new Set();
      if(!byKeyCategories[key])byKeyCategories[key]=new Set();
      byKey[key].add(tag);
      if(category)byKeyCategories[key].add(category);
    });
  });
  state.availableTags=[...tagSet].sort((a,b)=>a.localeCompare(b,'ja'));
  state.availableTagsByKey=Object.fromEntries(Object.entries(byKey).map(([key,set])=>[key,[...set].sort((a,b)=>a.localeCompare(b,'ja'))]));
  state.availableTagCategoriesByKey=Object.fromEntries(Object.entries(byKeyCategories).map(([key,set])=>[key,sortTagCategoryKeys([...set])]));
  state.availableTagGroupOrder=Object.keys(byKey);
  state.selectedTags=(state.selectedTags||[]).filter(tag=>tagSet.has(tag));
  rebuildAvailableTagCategoryIndex(all);
  rebuildTagCandidateSearchIndex();
...
function mergeClauseSearchTags(all){const rootApi=(typeof window!=='undefined'?window:globalThis);const api=rootApi.HADO_CLAUSE_SURFACE_BRIDGE||rootApi.HADO_SEARCH_CLAUSE_INTEGRATION;if(!api||typeof api.getEntityTags!=='function')return;const tagSet=new Set(state.availableTags||[]),byKey=Object.fromEntries(Object.entries(state.availableTagsByKey||{}).map(([key,values])=>[key,new Set(values)])),byCategory=Object.fromEntries(Object.entries(state.availableTagCategoriesByKey||{}).map(([key,values])=>[key,new Set(values)]));let appliedItems=0,applications=0;(Array.isArray(all)?all:[]).forEach(item=>{const category=detailCategory(item),tags=api.getEntityTags(category,getItemDisplayName(item));if(!tags.length)return;appliedItems++;item._detailTags=[...new Set([...(item._detailTags||getSearchTagsForItem(item)),...tags])];tags.forEach(tag=>{tagSet.add(tag);const p=tag.indexOf(':'),key=p>=0?tag.slice(0,p):'その他';if(!byKey[key])byKey[key]=new Set();if(!byCategory[key])byCategory[key]=new Set();byKey[key].add(tag);if(category)byCategory[key].add(category);applications++;});});state.availableTags=[...tagSet].sort((a,b)=>a.localeCompare(b,'ja'));state.availableTagsByKey=Object.fromEntries(Object.entries(byKey).map(([key,set])=>[key,[...set].sort((a,b)=>a.localeCompare(b,'ja'))]));state.availableTagCategoriesByKey=Object.fromEntries(Object.entries(byCategory).map(([key,set])=>[key,sortTagCategoryKeys([...set])]));const order=[...(state.availableTagGroupOrder||[])];['条件','発動'].forEach(key=>{if(byKey[key]&&!order.includes(key))order.push(key);});state.availableTagGroupOrder=order;if(state.diagnostics?.tagIndex)state.diagnostics.tagIndex.clauseIntegration={appliedItems,applications,groups:['条件','発動'].filter(key=>!!byKey[key])};}
function tagMatchesInput(tag,input){const q=norm(input).toLowerCase();if(!q)return true;const raw=norm(tag).toLowerCase(),key=displayTagGroupKey(getTagGroupKey(tag)).toLowerCase(),value=getTagValueLabel(tag).toLowerCase();return raw.includes(q)||value.includes(q)||`${key}:${value}`.includes(q);}
...
function ensureSelectedTagColorAssignments(){const api=window.HADO_TAG_HIGHLIGHT;if(!api)return {};state.selectedTagColorSlots=api.assignSlots(state.selectedTags||[],state.selectedTagColorSlots||{});return state.selectedTagColorSlots;}
function getTagColorSlot(tag){const slots=ensureSelectedTagColorAssignments();const value=Number(slots[norm(tag)]);return Number.isInteger(value)&&value>=0?value:0;}
function getTagColorClass(tag){return `tag-color-${getTagColorSlot(tag)}`;}
function getItemSearchTagSet(item){return new Set((item?._detailTags||getSearchTagsForItem(item)).map(norm).filter(Boolean));}
function getMatchedSelectedTags(item){const owned=getItemSearchTagSet(item);return (state.selectedTags||[]).map(norm).filter(tag=>tag&&owned.has(tag));}
function getSelectedTagHighlightEntries(item){const api=window.HADO_TAG_HIGHLIGHT;if(!api)return [];return api.buildEntries(state.selectedTags||[],[...getItemSearchTagSet(item)],getTagValueLabel,ensureSelectedTagColorAssignments());}
function highlightSelectedTagTextHtml(text,item){const api=window.HADO_TAG_HIGHLIGHT;return api?api.highlightTextHtml(text,getSelectedTagHighlightEntries(item)):esc(text);}
function highlightSelectedTagTextNodes(root,item){const api=window.HADO_TAG_HIGHLIGHT;if(!api||!root)return 0;const entries=getSelectedTagHighlightEntries(item);const count=api.applyToTextNodes(root,entries);if(count)debugLog('tagHighlight:detail',{category:detailCategory(item),name:getItemDisplayName(item),count,tags:entries.map(entry=>entry.tag)});return count;}
function renderMatchedSelectedTagsHtml(item){return getMatchedSelectedTags(item).map(tag=>`<span class="search-result-matched-tag tag-highlight-token ${getTagColorClass(tag)}" data-selected-tag="${esc(tag)}">${esc(displayTagGroupKey(getTagGroupKey(tag)))}：${esc(getTagValueLabel(tag))}</span>`).join('');}
function renderSelectedTagConditionChipsHtml(){return (state.selectedTags||[]).map(tag=>{const key=getTagGroupKey(tag);return `<span class="search-condition-chip is-tag tag-highlight-token ${getTagColorClass(tag)}" data-selected-tag="${esc(tag)}">タグ（${esc(getTagGroupCategoryLabel(key))}｜${esc(displayTagGroupKey(key))}）：${esc(getTagValueLabel(tag))}</span>`;}).join('');}
function rebuildTagCandidateSearchIndex(){const groupRank=new Map(getOrderedTagGroupKeys().map((key,index)=>[key,index]));state.tagCandidateIndex=(state.availableTags||[]).map(tag=>{const key=getTagGroupKey(tag),displayKey=displayTagGroupKey(key),label=getTagValueLabel(tag),rawLower=norm(tag).toLowerCase(),valueLower=label.toLowerCase(),displayLower=`${displayKey}:${label}`.toLowerCase();return {tag,key,label,categories:state.availableTagCategoriesByTag?.[tag]||[],rawLower,valueLower,displayLower,searchText:`${rawLower}\n${valueLower}\n${displayLower}`,groupRank:groupRank.get(key)??Number.MAX_SAFE_INTEGER};});debugLog('tagSearch:candidate-index',{count:state.tagCandidateIndex.length,source:'precomputed'});}
...
const TAG_DISPLAY_CATEGORY_ALIASES=Object.freeze({troop_skills:'troopSkills',status_effects:'statusEffects',siege_weapons:'siegeWeapons',ethnic_armaments:'ethnicArmaments',ethnic_research_skills:'ethnicResearchSkills',five_elements:'fiveElements',warhorse_skills:'warhorseSkills'});
function normalizeTagDisplayCategoryKey(key){const normalized=normalizeDerivedSearchCategory(key);return TAG_DISPLAY_CATEGORY_ALIASES[normalized]||normalized;}
function sortTagCategoryKeys(keys){const order=Array.isArray(SEARCH_CATEGORY_DISPLAY_ORDER)?SEARCH_CATEGORY_DISPLAY_ORDER:[];return [...new Set((Array.isArray(keys)?keys:[]).map(normalizeTagDisplayCategoryKey).filter(Boolean))].sort((a,b)=>{const ai=order.indexOf(a),bi=order.indexOf(b);if(ai!==bi){if(ai<0)return 1;if(bi<0)return -1;return ai-bi;}return (DATASET_LABELS[a]||a).localeCompare(DATASET_LABELS[b]||b,'ja');});}
function getTagGroupCategoryKeys(key){return sortTagCategoryKeys(state.availableTagCategoriesByKey?.[norm(key)]||[]);}
function getTagCategoryKeys(tag){return sortTagCategoryKeys(state.availableTagCategoriesByTag?.[norm(tag)]||[]);}
function tagAppliesToActiveCategories(tag){const categories=getTagCategoryKeys(tag);const active=Object.entries(state.activeCategories||{}).filter(([,enabled])=>!!enabled).map(([key])=>key);return active.length>0&&categories.some(category=>active.includes(category));}
function getVisibleTagOptionsForGroup(key){return (state.availableTagsByKey?.[norm(key)]||[]).filter(tagAppliesToActiveCategories);}
function getVisibleTagGroupCategoryKeys(key){const all=getTagGroupCategoryKeys(key);const active=all.filter(category=>!!state.activeCategories?.[category]);return active.length?active:all;}
function getTagGroupCategoryLabel(key){const labels=getVisibleTagGroupCategoryKeys(key).map(category=>DATASET_LABELS[category]||category);return labels.length?labels.join('・'):'共通';}
...
function getOrderedTagGroupKeys(){const keys=Object.keys(state.availableTagsByKey||{}).filter(key=>getVisibleTagOptionsForGroup(key).length>0).sort(compareTagGroupKeys);const statusIndex=keys.indexOf('状態変化'),skillIndex=keys.indexOf('技能');if(statusIndex>=0&&skillIndex>=0&&statusIndex!==skillIndex-1){keys.splice(statusIndex,1);keys.splice(keys.indexOf('技能'),0,'状態変化');}return keys;}
...
function refreshSearchAfterTagChange(){
  if(state.quickStatusEffectOwnerFilter&&typeof runQuickStatusEffectOwnerSearchAsync==='function'){
    ...
    runQuickStatusEffectOwnerSearchAsync(state.quickStatusEffectOwnerFilter,{reason:'tag-filter-change'});
  }
  renderSearchResults();
  renderDetail();
}
function scheduleSearchAfterTagChange(reason=''){
  state._tagSearchRefreshSeq=(state._tagSearchRefreshSeq||0)+1;
  const seq=state._tagSearchRefreshSeq;
  if(els.resultMeta)els.resultMeta.textContent='タグを反映しました。検索中…';
  const run=()=>{
    if(seq!==state._tagSearchRefreshSeq)return;
    debugLog('tagSearch:deferred-search',{reason,selectedTags:[...(state.selectedTags||[])],sequence:seq});
    refreshSearchAfterTagChange();
  };
...
function getTagCandidateScore(row,input){const q=norm(input).toLowerCase();if(row.rawLower===q)return 0;if(row.valueLower===q)return 1;if(row.displayLower===q)return 2;if(row.valueLower.startsWith(q))return 3;if(row.rawLower.startsWith(q)||row.displayLower.startsWith(q))return 4;return 5;}
function getMatchingTagCandidates(input,limit=16){const q=norm(input).toLowerCase();if(!q)return [];const selected=new Set(state.selectedTags||[]);return (state.tagCandidateIndex||[]).filter(row=>!selected.has(row.tag)&&tagAppliesToActiveCategories(row.tag)&&row.searchText.includes(q)).map(row=>({...row,score:getTagCandidateScore(row,q)})).sort((a,b)=>a.score-b.score||a.groupRank-b.groupRank||a.label.localeCompare(b.label,'ja')).slice(0,limit).map(row=>row.tag);}
function hideTagCandidates(){if(!els.tagSearchCandidates||!els.tagSearchInput)return;state.tagCandidateValues=[];state.tagCandidateActiveIndex=-1;state.tagCandidateSignature='';els.tagSearchCandidates.hidden=true;els.tagSearchCandidates.innerHTML='';els.tagSearchInput.setAttribute('aria-expanded','false');els.tagSearchInput.removeAttribute('aria-activedescendant');}
function selectTagCandidate(tag,reason='candidate-tap'){const value=norm(tag);if(!value)return false;if(els.tagSearchInput)els.tagSearchInput.value='';hideTagCandidates();const added=addSelectedTag(value,reason);if(els.tagSearchInput)els.tagSearchInput.focus({preventScroll:true});return added;}
function setTagCandidateActiveIndex(index){const values=state.tagCandidateValues||[];if(!values.length)return false;const next=(index+values.length)%values.length;state.tagCandidateActiveIndex=next;els.tagSearchCandidates?.querySelectorAll('[data-tag-candidate]').forEach((node,i)=>{const active=i===next;node.classList.toggle('is-active',active);node.setAttribute('aria-selected',active?'true':'false');if(active){els.tagSearchInput?.setAttribute('aria-activedescendant',node.id);node.scrollIntoView({block:'nearest'});}});return true;}
function moveTagCandidateActive(delta){const active=Number.isInteger(state.tagCandidateActiveIndex)?state.tagCandidateActiveIndex:-1;const current=active<0?(delta>0?-1:0):active;return setTagCandidateActiveIndex(current+delta);}
...
function renderSelectedTags(){if(!els.selectedTagList)return;const tags=state.selectedTags||[];ensureSelectedTagColorAssignments();els.selectedTagList.innerHTML='';if(!tags.length){els.selectedTagList.innerHTML='<span class="tag-search-note">タグ未指定</span>';return;}[...groupTagsByKey(tags).entries()].sort(([a],[b])=>compareTagGroupKeys(a,b)).forEach(([key,groupTags])=>{const group=document.createElement('span');group.className='selected-tag-group';const label=document.createElement('span');label.className='selected-tag-group-label';label.textContent=getTagGroupCategoryLabel(key)+'｜'+displayTagGroupKey(key)+'：';group.appendChild(label);groupTags.forEach(tag=>{const span=document.createElement('span');span.className=`selected-tag-badge tag-highlight-token ${getTagColorClass(tag)}`;span.dataset.selectedTag=tag;span.textContent=getTagValueLabel(tag);const btn=document.createElement('button');btn.type='button';btn.className='selected-tag-remove';btn.textContent='×';btn.setAttribute('aria-label',tag+' を解除');btn.addEventListener('click',()=>removeSelectedTag(tag,'badge'));span.appendChild(btn);group.appendChild(span);});els.selectedTagList.appendChild(group);});}
function bindTagCandidateInputOnce(){if(!els.tagSearchInput||els.tagSearchInput.dataset.tagCandidateBound==='1')return;els.tagSearchInput.dataset.tagCandidateBound='1';els.tagSearchInput.addEventListener('focus',()=>{state.tagCandidateBlurSeq=(state.tagCandidateBlurSeq||0)+1;renderTagCandidates();});els.tagSearchInput.addEventListener('blur',()=>{const seq=(state.tagCandidateBlurSeq||0)+1;state.tagCandidateBlurSeq=seq;setTimeout(()=>{if(seq!==state.tagCandidateBlurSeq||state.tagCandidatePointerDown||document.activeElement===els.tagSearchInput)return;hideTagCandidates();},180);});els.tagSearchInput.addEventListener('keydown',e=>{if(state.tagSearchComposing||e.isComposing||e.keyCode===229)return;if(e.key==='ArrowDown'||e.key==='ArrowUp'){if(!(state.tagCandidateValues||[]).length)renderTagCandidates();if(moveTagCandidateActive(e.key==='ArrowDown'?1:-1))e.preventDefault();return;}if(e.key==='Escape'){hideTagCandidates();return;}if(e.key==='Enter'&&state.tagCandidateActiveIndex>=0){const tag=(state.tagCandidateValues||[])[state.tagCandidateActiveIndex];if(tag){e.preventDefault();e.stopImmediatePropagation();selectTagCandidate(tag,'candidate-keyboard');}}},true);}
function renderTagCandidates(){if(!els.tagSearchCandidates||!els.tagSearchInput)return;bindTagCandidateInputOnce();if(state.tagSearchComposing){hideTagCandidates();return;}const started=typeof performance!=='undefined'&&performance.now?performance.now():Date.now(),q=norm(els.tagSearchInput.value);if(!q){hideTagCandidates();return;}const tags=getMatchingTagCandidates(q,16),signature=`${q}\u0000${tags.join('\u0000')}`;if(signature===state.tagCandidateSignature&&!els.tagSearchCandidates.hidden)return;state.tagCandidateSignature=signature;state.tagCandidateValues=tags;state.tagCandidateActiveIndex=-1;els.tagSearchCandidates.innerHTML='';tags.forEach((tag,index)=>{const option=document.createElement('button');option.type='button';option.id=`tagSearchCandidate-${index}`;option.className='tag-candidate-option';option.dataset.tagCandidate=tag;option.setAttribute('role','option');option.setAttribute('aria-selected','false');const value=document.createElement('span');value.className='tag-candidate-value';value.textContent=getTagValueLabel(tag);const group=document.createElement('span');group.className='tag-candidate-group';group.textContent=displayTagGroupKey(getTagGroupKey(tag));option.appendChild(value);option.appendChild(group);option.addEventListener('pointerdown',e=>{state.tagCandidatePointerDown=true;e.preventDefault();});option.addEventListener('pointercancel',()=>{state.tagCandidatePointerDown=false;});option.addEventListener('mouseenter',()=>setTagCandidateActiveIndex(index));option.addEventListener('click',()=>{state.tagCandidatePointerDown=false;selectTagCandidate(tag,'candidate-tap');});els.tagSearchCandidates.appendChild(option);});els.tagSearchCandidates.hidden=!tags.length;els.tagSearchInput.setAttribute('aria-expanded',tags.length?'true':'false');const elapsed=(typeof performance!=='undefined'&&performance.now?performance.now():Date.now())-started;debugLog('tagSearch:candidates-render',{input:q,count:tags.length,limit:16,elapsedMs:Number(elapsed.toFixed?elapsed.toFixed(1):elapsed),ui:'custom-listbox'});}
function renderTagPickerPanel(){if(!els.tagPickerPanel)return;const visible=!!state.tagPickerVisible;if(visible&&!state._tagPickerWasVisible)state.expandedTagPickerGroups=[];state._tagPickerWasVisible=visible;els.tagPickerPanel.classList.toggle('is-visible',visible);els.tagPickerPanel.innerHTML='';if(!visible){state.expandedTagPickerGroups=[];return;}const keys=getOrderedTagGroupKeys();if(!keys.length){els.tagPickerPanel.innerHTML='<div class="tag-search-note">タグ候補なし</div>';return;}const expandedKeys=new Set(state.expandedTagPickerGroups||[]);keys.forEach((key,index)=>{const expanded=expandedKeys.has(key);const group=document.createElement('section');group.className='tag-picker-group';group.dataset.tagCategories=getVisibleTagGroupCategoryKeys(key).join(' ');const optionsId=`tagPickerGroup-${index+1}`;const title=document.createElement('button');title.type='button';title.className='tag-picker-title';title.setAttribute('aria-expanded',expanded?'true':'false');title.setAttribute('aria-controls',optionsId);const category=document.createElement('span');category.className='tag-picker-category';category.textContent=getTagGroupCategoryLabel(key);const name=document.createElement('span');name.className='tag-picker-group-name';name.textContent=displayTagGroupKey(key)+'（グループ内OR）';const chevron=document.createElement('span');chevron.className='tag-picker-chevron';chevron.setAttribute('aria-hidden','true');chevron.textContent=expanded?'▲':'▼';title.appendChild(category);title.appendChild(name);title.appendChild(chevron);const options=document.createElement('div');options.id=optionsId;options.className='tag-picker-options';options.hidden=!expanded;getVisibleTagOptionsForGroup(key).forEach(tag=>{const label=document.createElement('label');const selected=(state.selectedTags||[]).includes(tag);label.className=`tag-picker-option${selected?` is-selected tag-highlight-token ${getTagColorClass(tag)}`:''}`;if(selected)label.dataset.selectedTag=tag;const cb=document.createElement('input');cb.type='checkbox';cb.value=tag;cb.checked=selected;cb.addEventListener('change',()=>{if(cb.checked)addSelectedTag(tag,'picker');else removeSelectedTag(tag,'picker');});label.appendChild(cb);label.appendChild(document.createTextNode(getTagValueLabel(tag)));options.appendChild(label);});title.addEventListener('click',()=>{const next=new Set(state.expandedTagPickerGroups||[]);if(next.has(key))next.delete(key);else next.add(key);state.expandedTagPickerGroups=[...next];renderTagPickerPanel();});group.appendChild(title);group.appendChild(options);els.tagPickerPanel.appendChild(group);});}
...
function matchesSelectedTags(item){const selected=state.selectedTags||[];if(!selected.length)return true;const tags=getItemSearchTagSet(item);return [...groupTagsByKey(selected).values()].every(groupTags=>groupTags.some(tag=>tags.has(tag)));}
