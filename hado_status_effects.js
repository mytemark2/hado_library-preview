...
function buildSearchTagIndex(){
  ...
  if(applyDerivedTagIndexToItems(all)){rebuildTagCandidateSearchIndex();mergeClauseSearchTags(all);rebuildTagCandidateSearchIndex();debugLog('tagSearch:indexBuilt',{source:'hadou_tag_index.json+EffectClause',totalTags:state.availableTags.length,keys:Object.keys(state.availableTagsByKey),sample:state.availableTags.slice(0,20),diagnostic:state.diagnostics.tagIndex});renderTagSearchControls();return;}
