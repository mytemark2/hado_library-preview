...
function getResultCardBadgesHtml(row){const card=getResultCardIndexForRow(row);const badges=Array.isArray(card?.badges)?card.badges.map(norm).filter(Boolean).filter(isAllowedSearchResultBadge).slice(0,4):[];const datasetHtml=badges.map(v=>`<span class="search-result-mini-badge is-dataset">${esc(v)}</span>`).join('');const clauseApi=(typeof window!=='undefined'?window:globalThis).HADO_SEARCH_CLAUSE_INTEGRATION;const clauseHtml=clauseApi&&typeof clauseApi.renderResultHtml==='function'?clauseApi.renderResultHtml({category:row?.key,name:getItemDisplayName(row?.item),statusFilter:state.quickStatusEffectOwnerFilter}):'';return datasetHtml+clauseHtml;}
...
function getFastSearchableText(item){
  if(!item)return '';
  const clauseApi=(typeof window!=='undefined'?window:globalThis).HADO_SEARCH_CLAUSE_INTEGRATION;const clauseText=clauseApi&&typeof clauseApi.getEntitySearchText==='function'?clauseApi.getEntitySearchText(detailCategory(item),getItemDisplayName(item)):'';
