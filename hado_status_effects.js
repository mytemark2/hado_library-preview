...
function removeSelectedTag(tag,reason=''){const t=norm(tag);state.selectedTags=(state.selectedTags||[]).filter(v=>v!==t);debugLog('tagSearch:selected',{reason,selectedTags:state.selectedTags,selectedTagGroups:getSelectedTagGroupsDebug(),groupRelation:'AND'});renderTagSearchControls();refreshSearchAfterTagChange();pushOperationHistory('tag-remove');}
function clearSelectedTags(reason=''){state.selectedTags=[];if(els.tagSearchInput)els.tagSearchInput.value='';debugLog('tagSearch:selected',{reason,selectedTags:state.selectedTags,selectedTagGroups:[],groupRelation:'AND'});renderTagSearchControls();refreshSearchAfterTagChange();pushOperationHistory('tag-clear');}
