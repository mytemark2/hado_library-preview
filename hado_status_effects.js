...
function buildSearchTagIndex(){
  ...
  if(applyDerivedTagIndexToItems(all)){debugLog('tagSearch:indexBuilt',{source:'hadou_tag_index.json',totalTags:state.availableTags.length,keys:Object.keys(state.availableTagsByKey),sample:state.availableTags.slice(0,20),diagnostic:state.diagnostics.tagIndex});renderTagSearchControls();return;}
...
function getTagCandidateScore(tag,input){const q=norm(input).toLowerCase(),raw=norm(tag).toLowerCase(),value=getTagValueLabel(tag).toLowerCase(),display=`${displayTagGroupKey(getTagGroupKey(tag))}:${value}`.toLowerCase();if(raw===q)return 0;if(value===q)return 1;if(display===q)return 2;if(value.startsWith(q))return 3;if(raw.startsWith(q)||display.startsWith(q))return 4;return 5;}
function getMatchingTagCandidates(input,limit=16){const q=norm(input);if(!q)return [];return (state.availableTags||[]).filter(tag=>!(state.selectedTags||[]).includes(tag)&&tagMatchesInput(tag,q)).map(tag=>({tag,score:getTagCandidateScore(tag,q)})).sort((a,b)=>a.score-b.score||compareTagGroupKeys(getTagGroupKey(a.tag),getTagGroupKey(b.tag))||getTagValueLabel(a.tag).localeCompare(getTagValueLabel(b.tag),'ja')).slice(0,limit).map(row=>row.tag);}
function resolveAvailableTagInput(value){const input=norm(value);if(!input)return '';if(state.availableTags.includes(input))return input;const q=input.toLowerCase();const exact=(state.availableTags||[]).filter(tag=>{const key=displayTagGroupKey(getTagGroupKey(tag)),label=getTagValueLabel(tag);return label.toLowerCase()===q||`${key}:${label}`.toLowerCase()===q;});return exact.length===1?exact[0]:'';}
