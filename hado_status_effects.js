...
function isSearchExcludedTable(item,table){const category=norm(item?.sourceDataset||item?.category||'');if(!table)return false;const rows=Array.isArray(table)?table:(Array.isArray(table?.rows)?table.rows:[]);if(!rows.length)return false;if(category==='generals'&&[20,21].includes(Number(table?.index)))return true;const firstRow=Array.isArray(rows[0])?rows[0]:[];const first0=norm(firstRow[0]||'');const first1=norm(firstRow[1]||'');if(category==='equipments'&&first0==='武将')return true;if(category==='generals'&&first0==='名宝'&&first1==='技能効果')return true;return false;}
function isSearchExcludedTable(item,table){const category=norm(item?.sourceDataset||item?.category||'');if(!table)return false;const rows=Array.isArray(table)?table:(Array.isArray(table?.rows)?table.rows:[]);if(!rows.length)return false;if(category==='generals'&&[20,21].includes(Number(table?.index)))return true;const firstRow=Array.isArray(rows[0])?rows[0]:[];const first0=norm(firstRow[0]||'');const first1=norm(firstRow[1]||'');if(category==='equipments'&&first0==='武将')return true;if(category==='generals'&&first0==='名宝'&&first1==='技能効果')return true;return false;}
