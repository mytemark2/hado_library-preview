...
function calculateFormationAutoScores(f,data={}){const generals=countFormationFilledGenerals(f);const equipments=countFormationFilledEquipments(f);const attendants=countFormationFilledAttendants(f);const advisors=countFormationFilledAdvisors(f);const extensionCount=(normalizeSaveItemName(f?.siegeWeapon?.name||'')?1:0)+(normalizeSaveItemName(f?.ethnicArmament?.name||'')?1:0);const skillCount=Array.isArray(data?.skills)?data.skills.length:0;const parameterCount=Object.keys(data?.parameterSummary||{}).length||0;const generalScore=generals/FORMATION_SLOT_SPECS.length*3;const equipmentScore=equipments/(FORMATION_SLOT_SPECS.length*EQUIP_SLOT_SPECS.length)*2;const attendantScore=attendants/FORMATION_SLOT_SPECS.length*1.5;const advisorScore=advisors/Math.max(1,Object.keys(createFormationAdvisorSlots()).length)*1;const extensionScore=extensionCount/2*1;const skillScore=Math.min(1.5,skillCount/12*1.5);const totalScore=Math.max(0,Math.min(10,Math.round((generalScore+equipmentScore+attendantScore+advisorScore+extensionScore+skillScore)*10)/10));const typeBonus=norm(f?.evaluationTypeName||'')?0.6:0;const effectBonus=Math.min(1.4,parameterCount/8);const evaluationScore=Math.max(0,Math.min(10,Math.round(Math.min(10,totalScore+typeBonus+effectBonus)*10)/10));return {totalScore,evaluationScore,breakdown:{generals,equipments,attendants,advisors,extensionCount,skillCount,parameterCount,generalScore,equipmentScore,attendantScore,advisorScore,extensionScore,skillScore,typeBonus,effectBonus}};}
function renderFormationScoreSummaryHtml(f,data){const scores=calculateFormationAutoScores(f,data);f.totalScore=scores.totalScore;f.evaluationScore=scores.evaluationScore;const b=scores.breakdown;const rows=[['武将',`${b.generals}/${FORMATION_SLOT_SPECS.length}`],['装備',`${b.equipments}/${FORMATION_SLOT_SPECS.length*EQUIP_SLOT_SPECS.length}`],['侍従',`${b.attendants}/${FORMATION_SLOT_SPECS.length}`],['参軍',`${b.advisors}/${Object.keys(createFormationAdvisorSlots()).length}`],['兵器/武装',`${b.extensionCount}/2`],['合算技能',`${b.skillCount}件`],['状態変化',`${b.parameterCount}件`],['型',formationEvaluationTypeDisplayName(f)]];return `<div class="formation-score-summary" aria-label="自動計算スコア"><div class="formation-score-total"><span>トータルスコア</span><strong>${esc(scores.totalScore)}/10</strong></div><div class="formation-score-sub"><span>評価スコア</span><strong>${esc(scores.evaluationScore)}/10</strong></div><div class="formation-score-breakdown">${rows.map(([label,value])=>`<span><span class="formation-score-label">${esc(label)}</span>${esc(value)}</span>`).join('')}</div></div>`;}
function renderFormationGroupNameDialogHtml(){if(!state.formationGroupNameDialogOpen)return '';const group=getCurrentFormationGroup();return `<div class="formation-mobile-dialog-overlay formation-group-dialog-overlay" data-formation-dialog-backdrop="group-name"><div class="formation-mobile-dialog-card" role="dialog" aria-modal="true" aria-label="グループ名変更"><div class="formation-mobile-dialog-head"><div class="formation-mobile-dialog-title">グループ名変更</div><button type="button" class="formation-mobile-dialog-close" data-formation-group-name-close="1">閉じる</button></div><label class="formation-dialog-field"><span class="note">グループ名</span><input id="formationGroupNameDialogInput" type="text" value="${esc(group.name)}" maxlength="32"></label><div class="formation-dialog-actions"><button type="button" data-formation-group-name-close="1">キャンセル</button><button type="button" id="formationGroupNameSaveBtn" class="btn-select-all">変更を保存</button></div></div></div>`;}
...
function renderFormationWarhorseSlotsHtml(){const {data,active}=getFormationWarhorseActiveSlots();const owned=Object.values(data.owned||{}).sort((a,b)=>norm(a.name||a.customName||a.id).localeCompare(norm(b.name||b.customName||b.id),'ja'));const options=(selected)=>`<option value="">未設定</option>${owned.map(w=>`<option value="${esc(w.id)}" ${norm(selected)===norm(w.id)?'selected':''}>${esc(w.name||w.customName||w.id)}</option>`).join('')}`;const rows=[0,1,2].map(i=>{const id=norm(active[i]||'');return `<div class="formation-warhorse-slot-row"><label><span class="note">軍馬${i+1}</span><select class="formation-select" data-formation-warhorse-slot="${i}">${options(id)}</select></label><div class="formation-warhorse-slot-actions"><button type="button" data-formation-warhorse-edit="${i}" ${id?'':'disabled'}>編集</button><button type="button" data-formation-warhorse-remove="${i}" ${id?'':'disabled'}>解除</button></div></div>`;}).join('');const emptyNote=owned.length?'':'<div class="formation-note">登録済み軍馬がありません。軍馬編成画面で軍馬を登録してください。</div>';return `<div class="formation-warhorse-slots"><div class="formation-section-header"><h3>軍馬</h3><span class="note">保存データの軍馬を最大3枠まで部隊へ反映</span></div>${emptyNote}${rows}</div>`;}
