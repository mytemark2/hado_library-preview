...
function formationScoreRowToDebug(row){return {featureId:row?.featureId||'',label:row?.label||row?.statusEffectName||'',sourceKind:row?.sourceKind||'',sourceLabel:row?.sourceLabel||'',key:row?.key||'',group:row?.group||'',timing:row?.timing||'',value:row?.value||'',matchedText:String(row?.matchedText||'').slice(0,160)};}
function buildFormationEffectScoreEntity(f,data={}){const parameterRows=collectFormationAllParameterRows(data);const effects=Array.isArray(data?.effects)?data.effects:[];const parameterFeatures=parameterRows.map(r=>({featureId:`parameter:${norm(r.key||r.label||'')}`,label:r.label||r.key||'',statusEffectName:r.label||r.key||'',matchedText:[r.label||'',r.key||'',r.timingLabel||'',r.group||'',r.value||''].filter(Boolean).join(' '),sourceKind:'parameter',key:r.key||'',group:r.group||'',timing:r.timing||'',value:r.value||''}));const statusRefs=effects.map(e=>({featureId:`status_effect:${norm(e?.key||'')}`,label:e?.key||'',statusEffectName:e?.key||'',matchedText:[e?.key||'',e?.group||'',e?.timing||'',e?.rawText||'',e?.condition||'',e?.sourceLabel||'',effectSignedValue(e)].filter(Boolean).join(' '),sourceKind:'effect',sourceLabel:e?.sourceLabel||'',key:e?.key||'',group:e?.group||'',timing:e?.timing||'',value:effectSignedValue(e)}));return {roleId:'formation_effects',displayName:f?.name||'部隊',typeFeatures:parameterFeatures,statusEffectRefs:statusRefs,parameterRows,effects};}
...
function renderFormationScoreEvidenceItems(items,label){const rows=(Array.isArray(items)?items:[]).slice(0,6);if(!rows.length)return '';return `<div class="formation-score-evidence-group"><div class="formation-score-evidence-title">${esc(label)}</div>${rows.map(row=>`<div class="formation-score-evidence-row"><span>${esc(row.sourceLabel||row.label||row.key||'一致')}</span><b>${esc(row.value||'')}</b></div>`).join('')}</div>`;}
function renderFormationScoreRowChipHtml(row){const effects=Array.isArray(row?.matchedEffects)?row.matchedEffects:[],parameters=Array.isArray(row?.matchedParameters)?row.matchedParameters:[];const hasDetails=effects.length||parameters.length;if(!hasDetails)return `<span class="formation-score-chip"><span class="formation-score-label">${esc(row.label)}</span><span class="value">${esc(row.score)}</span></span>`;return `<details class="formation-score-chip formation-score-chip-detail"><summary><span class="formation-score-label">${esc(row.label)}</span><span class="value">${esc(row.score)}</span></summary><div class="formation-score-chip-detail-body">${renderFormationScoreEvidenceItems(effects,'効果')}${renderFormationScoreEvidenceItems(parameters,'変化率')}</div></details>`;}
...
function renderFormationScoreSummaryHtml(f,data){
  ...
  return `<details class="formation-score-summary" aria-label="自動計算スコア"${openAttr}><summary class="formation-score-summary-head"><div class="formation-score-total"><span>トータルスコア</span><strong>${esc(scores.totalScore)}</strong></div><span class="formation-score-toggle-note">評価スコアを表示</span></summary><div class="formation-score-summary-body"><div class="formation-score-breakdown formation-score-generals">${rows.map(renderFormationScoreRowChipHtml).join('')}</div>${candidateRows}<div class="formation-score-breakdown formation-score-meta">${meta.map(([label,value])=>`<span><span class="formation-score-label">${esc(label)}</span>${esc(value)}</span>`).join('')}</div>${emptyNote}</div></details>`;
}
