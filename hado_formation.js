...
function formationScoreRowToDebug(row){return {featureId:row?.featureId||'',label:row?.label||row?.statusEffectName||'',sourceKind:row?.sourceKind||'',sourceLabel:row?.sourceLabel||'',key:row?.key||'',group:row?.group||'',timing:row?.timing||'',value:row?.value||'',condition:row?.condition||'',changeItemId:row?.changeItemId||'',scoreRole:row?.scoreRole||'',excludeReason:row?.excludeReason||'',targetScope:row?.targetScope||'',effectFamily:row?.effectFamily||'',matchedText:String(row?.matchedText||'').slice(0,240),rawText:String(row?.rawText||row?.matchedText||'').slice(0,500)};}
...
function normalizeFormationScoreDisplayRows(rows){const src=Array.isArray(rows)?rows:[];const out=src.slice(0,5).map((row,index)=>({label:row?.label||FORMATION_SCORE_FALLBACK_LABELS[index]||`評価${index+1}`,score:Number(row?.score||0),unit:row?.unit||'件',matchedEffects:Array.isArray(row?.matchedEffects)?row.matchedEffects:[],matchedParameters:Array.isArray(row?.matchedParameters)?row.matchedParameters:[],scoreDetails:Array.isArray(row?.scoreDetails)?row.scoreDetails:[],excludedRows:Array.isArray(row?.excludedRows)?row.excludedRows:[],zeroPrimaryReason:row?.zeroPrimaryReason||'',zeroPrimaryMessage:row?.zeroPrimaryMessage||''}));while(out.length<5){const idx=out.length;out.push({label:FORMATION_SCORE_FALLBACK_LABELS[idx]||`評価${idx+1}`,score:0,unit:'件',matchedEffects:[],matchedParameters:[],scoreDetails:[],excludedRows:[],zeroPrimaryReason:'',zeroPrimaryMessage:''});}return out;}
...
const FORMATION_SCORE_CHANGE_ITEM_LABELS={tactic_speed:'戦法速度',tactic_gauge:'戦法ゲージ',normal_attack_target_count_up:'通常攻撃対象数',attack_speed_up:'攻撃速度',defense_up:'防御',healing:'兵力回復',wounded_recovery:'負傷兵回復',wounded_survival:'負傷兵生存',attribute_resistance:'属性耐性',tactic_power_up:'戦法威力',normal_attack_power_up:'通常攻撃威力',weakening_nullify:'弱化無効',weakening_remove:'弱化解除',status_nullify:'状態変化無効',status_remove:'状態異常解除',severance_counter:'分断対策',isolation_counter:'絶縁対策',chain_nullify_counter:'連鎖無効対策',buff_protection:'強化解除・奪取対策'};
...
function formationScoreChangeItemLabel(row){const id=row?.changeItemId||row?.effectFamily||'';return FORMATION_SCORE_CHANGE_ITEM_LABELS[id]||row?.label||row?.key||'その他';}
...
function renderFormationScoreExcludedSummaryHtml(row,expanded=false){const excluded=Array.isArray(row?.excludedRows)?row.excludedRows:[];if(!excluded.length)return '';const groups=formationScoreExcludedGroups(excluded);const total=excluded.length;const reasonText=Object.entries(formationScoreExcludedReasonCounts(excluded)).sort((a,b)=>b[1]-a[1]).slice(0,2).map(([label,count])=>`${esc(label)} ${esc(count)}件`).join('、');if(!expanded)return `<div class="formation-score-zero-primary is-summary" data-formation-score-zero-primary="1"><p><strong>評価対象Primaryなし・除外根拠あり</strong></p><p>不利対策Primaryはありません。防御・回復・戦法ゲージ・攻撃速度など${esc(total)}件はワクチン型の評価対象外です。</p><p class="formation-note">詳細表示で除外カテゴリを確認できます。</p></div>`;const groupList=groups.map(group=>{const reason=Object.entries(group.reasons).sort((a,b)=>b[1]-a[1])[0]?.[0]||'評価対象外';const samples=group.samples.map(sample=>`<li>${esc(sample.sourceLabel)} / ${esc(sample.reason)} / ${esc(sample.targetScope)}</li>`).join('');return `<li><b>${esc(group.label)}</b>: ${esc(group.count)}件 <span>${esc(reason)}</span>${samples?`<ul>${samples}</ul>`:''}</li>`;}).join('');return `<div class="formation-score-zero-primary is-detail" data-formation-score-zero-primary="1"><p><strong>評価対象Primaryなし・除外根拠あり</strong></p><p>不利対策Primaryはありません。除外された根拠 ${esc(total)}件をカテゴリ別に集約しています。</p><p class="formation-note">主な除外理由: ${reasonText||'評価対象外'}</p><ul>${groupList}</ul></div>`;}
function renderFormationScoreEvidencePanelHtml(row){if(!row)return '';const evidenceRows=normalizeFormationScoreEvidenceRows(row);const summaryRows=uniqueFormationScoreEvidenceSummaryRows(evidenceRows);const excludedHtml=renderFormationScoreExcludedSummaryHtml(row,false);const body=summaryRows.length?`<div class="formation-score-evidence-tags is-collapsed" aria-label="${esc(row.label||'評価スコア')}の簡略根拠">${renderFormationScoreEvidenceItemsHtml(summaryRows,false)}</div>`:(excludedHtml||'<div class="formation-score-empty">該当タグなし</div>');return `<div class="formation-score-detail-panel is-collapsed is-clickable" data-formation-score-detail-open="1" role="button" tabindex="0" aria-label="${esc(row.label||'評価スコア')}の詳細を表示"><div class="formation-score-detail-head"><strong>${esc(row.label)}の内訳</strong><span>クリックで詳細表示</span></div>${body}</div>`;}
function renderFormationScoreEvidenceDialogHtml(row){if(!state.formationScoreEvidenceDialogOpen||!row)return '';const evidenceRows=normalizeFormationScoreEvidenceRows(row);const excludedHtml=renderFormationScoreExcludedSummaryHtml(row,true);return `<div class="formation-mobile-dialog-overlay formation-score-evidence-dialog-overlay" data-formation-score-dialog-backdrop="1"><div class="formation-mobile-dialog-card formation-score-evidence-dialog-card" role="dialog" aria-modal="true" aria-label="${esc(row.label||'評価スコア')}の内訳"><div class="formation-mobile-dialog-head"><div class="formation-mobile-dialog-title">${esc(row.label||'評価スコア')}の内訳</div><button type="button" class="formation-mobile-dialog-close" data-formation-score-dialog-close="1">閉じる</button></div><div class="formation-score-detail-head"><strong>評価${esc(evidenceRows.length)} / 根拠${esc(evidenceRows.length)}件</strong><span>全件表示</span></div>${evidenceRows.length?`<div class="formation-score-evidence-tags is-expanded formation-score-evidence-dialog-list" aria-label="${esc(row.label||'評価スコア')}の根拠全件">${renderFormationScoreEvidenceItemsHtml(evidenceRows,true)}</div>`:(excludedHtml||'<div class="formation-score-empty">該当タグなし</div>')}<div class="formation-note">結果サマリーと同じチップ形式で、選択中評価スコアの根拠を全件表示しています。</div></div></div>`;}
...
function renderFormationScoreSummaryHtml(f,data){
  ...
  const emptyReason=scores.breakdown.emptyReason||(!matchedCount?'評価対象Primaryなし・除外根拠あり':'');
