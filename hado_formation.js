...
const FORMATION_SCORE_SOURCE_TYPE_LABELS={skill:'技能',tactic:'戦法',statusEffect:'状態変化',equipment:'装備',formation:'陣形',fiveElement:'五行',warhorseSkill:'軍馬技能',ethnicResearchSkill:'異文化調査技能'};
function formationScoreSourceDisplayLabel(sourceLabel,sourceType=''){const raw=String(sourceLabel||'').trim();if(!raw||raw==='スコア根拠')return raw||'スコア根拠';const parts=raw.split(/[:：]/).map(v=>v.trim()).filter(Boolean);const owner=parts[0]||raw;const kind=FORMATION_SCORE_SOURCE_TYPE_LABELS[sourceType]||parts.find(v=>['戦法','技能','装備','陣形','五行','軍馬技能','異文化調査技能','状態変化'].includes(v))||'';return kind?`${owner} / ${kind}`:owner;}
...
const FORMATION_SCORE_EXCLUDE_REASON_LABELS={deny_change_item:'この型では評価しない項目',target_not_allowed:'対象がこの型の条件外',unknown_target_for_target_dependent_type:'対象が判定できないため除外',duplicate_evidence:'同一起源の重複根拠',support_without_primary:'Primary不足のため補助効果のみでは評価不可',source_type_not_allowed:'根拠種別がこの型の条件外',timing_not_allowed:'発動タイミングがこの型の条件外',aggregate_or_derived_origin:'集計・派生根拠のため直接評価対象外',tactic_source_inactive_slot:'戦法由来効果は主将・副将1・副将2のみ有効'};
