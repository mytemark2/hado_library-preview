...
function getFormationTypeScoreRule(f){const typeId=norm(f?.evaluationTypeId||'');const typeName=norm(f?.evaluationTypeName||'');return getFormationTypeScoreRules().find(rule=>norm(rule?.typeId||'')===typeId)||(typeName?getFormationTypeScoreRules().find(rule=>norm(rule?.typeName||'')===typeName):null)||null;}
function formationTypeScoreEntity(f,data={}){const effects=Array.isArray(data?.effects)?data.effects:[];return {roleId:'formation',displayName:f?.name||'部隊',typeFeatures:effects.map(e=>({featureId:`parameter:${norm(e?.key||'')}`,label:e?.key||'',statusEffectName:e?.key||'',matchedText:[e?.key||'',e?.rawText||'',e?.condition||'',e?.sourceLabel||''].filter(Boolean).join(' ')})),statusEffectRefs:[]};}
function getFormationTypeSearchFeatureItems(){const bucket=state?.derivedData?.typeSearchFeatureIndex;return bucket&&bucket.available&&Array.isArray(bucket.items)?bucket.items:[];}
...
function formationMemberScoreEntities(f){return collectFormationItems(f).map(row=>{const category=row.sourceType==='equipment'?'equipments':'generals';const entry=findFormationTypeSearchFeatureEntry(row.item,category);if(!entry)return null;return {roleId:formationTypeScoreRoleForCollectedItem(row),displayName:row.holder||getItemDisplayName(row.item),slotLabel:row.slotLabel||row.kind||'',typeFeatures:Array.isArray(entry.typeFeatures)?entry.typeFeatures:[],statusEffectRefs:Array.isArray(entry.statusEffectRefs)?entry.statusEffectRefs:[]};}).filter(v=>v&&(v.typeFeatures.length||v.statusEffectRefs.length));}
function calculateFormationMemberScoreRows(f,rule){if(!rule||!window.HadoTypeScore)return null;const metrics=Array.isArray(rule?.metrics)?rule.metrics.slice(0,5):[];const members=formationMemberScoreEntities(f);if(!metrics.length||!members.length)return null;const scoreRows=metrics.map(metric=>({label:metric.label||metric.metricKey||'',score:0,unit:'件'}));let totalScore=0;members.forEach(member=>{const memberScore=window.HadoTypeScore.score(member,rule);totalScore+=Number(memberScore.fitScore||0);(memberScore.breakdown||[]).slice(0,5).forEach((row,index)=>{if(scoreRows[index])scoreRows[index].score+=Number(row.itemCount||0);});});const evaluationScore=scoreRows.reduce((sum,row)=>sum+Number(row.score||0),0);debugLog('formation:type-score-member-aggregate',{formationId:f?.id||'',formationName:f?.name||'',typeName:rule?.typeName||'',memberCount:members.length,totalScore,evaluationScore,scoreRows});return {totalScore,evaluationScore,scoreRows,memberCount:members.length};}
...
function calculateFormationAutoScores(f,data={}){ensureFormationTypeScoreRulesLoaded();const rule=getFormationTypeScoreRule(f);const skillCount=Array.isArray(data?.skills)?data.skills.length:0;const parameterCount=Array.isArray(data?.effects)?data.effects.length:(Object.keys(data?.parameterSummary||{}).length||0);if(rule&&window.HadoTypeScore){const memberScores=calculateFormationMemberScoreRows(f,rule);if(memberScores){return {totalScore:memberScores.totalScore,evaluationScore:memberScores.evaluationScore,breakdown:{scoreRows:memberScores.scoreRows,filledGenerals:countFormationFilledGenerals(f),skillCount,parameterCount,evaluationTypeName:formationEvaluationTypeDisplayName(f),scorePolicy:'type-member-matched-item-count',memberCount:memberScores.memberCount}};}const score=window.HadoTypeScore.score(formationTypeScoreEntity(f,data),rule);const scoreRows=(score.breakdown||[]).map(row=>({label:row.label||row.metricKey||'',score:Number(row.itemCount||0),unit:'件'}));return {totalScore:score.totalScore||0,evaluationScore:score.evaluationScore||0,breakdown:{scoreRows,filledGenerals:0,skillCount,parameterCount,evaluationTypeName:formationEvaluationTypeDisplayName(f),scorePolicy:'type-matched-item-count'}};}const generalRows=calculateFormationGeneralScoreRows(f,data).map(row=>({...row,unit:'件'}));const totalScore=0,evaluationScore=0;return {totalScore,evaluationScore,breakdown:{scoreRows:generalRows.map(row=>({label:row.label,name:row.name,score:0,unit:'件'})),filledGenerals:generalRows.filter(row=>row.name&&row.name!=='未設定').length,skillCount,parameterCount,evaluationTypeName:formationEvaluationTypeDisplayName(f),scorePolicy:'waiting-type-score-rules'}};}
function renderFormationScoreSummaryHtml(f,data){
  ...
  const rows=scores.breakdown.scoreRows||[];
  ...
  const openAttr=isResponsiveMobileMode()?'':' open';
  return `<details class="formation-score-summary" aria-label="自動計算スコア"${openAttr}><summary class="formation-score-summary-head"><div class="formation-score-total"><span>トータルスコア</span><strong>${esc(scores.totalScore)}</strong></div><div class="formation-score-sub"><span>評価スコア</span><strong>${esc(scores.evaluationScore)}</strong></div><span class="formation-score-toggle-note">評価項目を表示</span></summary><div class="formation-score-summary-body"><div class="formation-score-breakdown formation-score-generals">${rows.map(row=>`<span><span class="formation-score-label">${esc(row.label)}</span>${esc(row.score)}</span>`).join('')}</div><div class="formation-score-breakdown formation-score-meta">${meta.map(([label,value])=>`<span><span class="formation-score-label">${esc(label)}</span>${esc(value)}</span>`).join('')}</div></div></details>`;
}
...
function renderFormationWarhorseSlotsHtml(){const {data,active}=getFormationWarhorseActiveSlots();const owned=Object.values(data.owned||{}).sort((a,b)=>norm(a.name||a.customName||a.id).localeCompare(norm(b.name||b.customName||b.id),'ja'));const options=(selected)=>`<option value="">未設定</option>${owned.map(w=>`<option value="${esc(w.id)}" ${norm(selected)===norm(w.id)?'selected':''}>${esc(w.name||w.customName||w.id)}</option>`).join('')}`;const rows=[0,1,2].map(i=>{const id=norm(active[i]||'');return `<div class="formation-warhorse-slot-row"><label><span class="note">軍馬${i+1}</span><select class="formation-select" data-formation-warhorse-slot="${i}">${options(id)}</select></label></div>`;}).join('');const emptyNote=owned.length?'':'<div class="formation-note">登録済み軍馬がありません。軍馬編成画面で軍馬を登録してください。</div>';return `<div class="formation-warhorse-slots"><div class="formation-section-header"><h3>軍馬</h3><span class="note">保存データの軍馬を最大3枠まで部隊へ反映</span></div>${emptyNote}<div class="formation-warhorse-slots-body">${rows}</div></div>`;}
