...
function calculateFormationAutoScores(f,data={}){ensureFormationTypeScoreRulesLoaded();const rule=getFormationTypeScoreRule(f);const skillCount=Array.isArray(data?.skills)?data.skills.length:0;const parameterCount=Array.isArray(data?.effects)?data.effects.length:(Object.keys(data?.parameterSummary||{}).length||0);if(rule&&window.HadoTypeScore){const memberScores=calculateFormationMemberScoreRows(f,rule);if(memberScores){return {totalScore:memberScores.totalScore,evaluationScore:memberScores.evaluationScore,breakdown:{scoreRows:memberScores.scoreRows,filledGenerals:countFormationFilledGenerals(f),skillCount,parameterCount,evaluationTypeName:formationEvaluationTypeDisplayName(f),scorePolicy:'type-member-matched-item-count',memberCount:memberScores.memberCount}};}const scoreRows=(Array.isArray(rule?.metrics)?rule.metrics.slice(0,5):[]).map(metric=>({label:metric.label||metric.metricKey||'',score:0,unit:'件'}));debugLog('formation:type-score-member-unavailable',{formationId:f?.id||'',formationName:f?.name||'',typeName:rule?.typeName||'',scoreRows:scoreRows.map(r=>r.label),policy:'no formationTypeScoreEntity fallback'});return {totalScore:0,evaluationScore:0,breakdown:{scoreRows,filledGenerals:countFormationFilledGenerals(f),skillCount,parameterCount,evaluationTypeName:formationEvaluationTypeDisplayName(f),scorePolicy:'type-member-score-unavailable'}};}const scoreRows=[];return {totalScore:0,evaluationScore:0,breakdown:{scoreRows,filledGenerals:countFormationFilledGenerals(f),skillCount,parameterCount,evaluationTypeName:formationEvaluationTypeDisplayName(f),scorePolicy:'waiting-type-score-rules'}};}
