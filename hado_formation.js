...
function buildFormationEffectScoreEntity(f,data={}){const parameterRows=collectFormationAllParameterRows(data);const effects=Array.isArray(data?.effects)?data.effects:[];const parameterFeatures=parameterRows.map(r=>({featureId:`parameter:${norm(r.key||r.label||'')}`,label:r.label||r.key||'',statusEffectName:r.label||r.key||'',matchedText:[r.label||'',r.key||'',r.timingLabel||'',r.group||'',r.value||''].filter(Boolean).join(' '),sourceKind:'parameter',key:r.key||'',group:r.group||'',timing:r.timing||'',value:r.value||''}));const effectFeatures=effects.map(e=>({featureId:`parameter:${norm(e?.key||'')}`,label:e?.key||'',statusEffectName:e?.key||'',matchedText:[e?.key||'',e?.group||'',e?.timing||'',e?.rawText||'',e?.condition||'',e?.sourceLabel||'',effectSignedValue(e)].filter(Boolean).join(' '),sourceKind:'effect',sourceLabel:e?.sourceLabel||'',key:e?.key||'',group:e?.group||'',timing:e?.timing||'',value:effectSignedValue(e)}));const statusRefs=effects.map(e=>({featureId:`status_effect:${norm(e?.key||'')}`,label:e?.key||'',statusEffectName:e?.key||'',matchedText:[e?.key||'',e?.group||'',e?.timing||'',e?.rawText||'',e?.condition||'',e?.sourceLabel||'',effectSignedValue(e)].filter(Boolean).join(' '),sourceKind:'effect',sourceLabel:e?.sourceLabel||'',key:e?.key||'',group:e?.group||'',timing:e?.timing||'',value:effectSignedValue(e)}));return {roleId:'formation_effects',displayName:f?.name||'部隊',typeFeatures:[...parameterFeatures,...effectFeatures],statusEffectRefs:statusRefs,parameterRows,effects};}
...
function calculateFormationAutoScores(f,data={}){const diag=calculateFormationTypeScore(f,data);const selected=diag.candidateScores?.[0]||null;const scoreRows=(selected?.rows||[]).slice(0,5).map(row=>({label:row.label,score:row.score,unit:'件'}));const totalScore=selected?Number(selected.totalScore||0):0;return {totalScore,evaluationScore:totalScore,breakdown:{scoreRows,filledGenerals:countFormationFilledGenerals(f),skillCount:Array.isArray(data?.skills)?data.skills.length:0,parameterCount:diag.effectSourceCount||diag.parameterRowCount||0,evaluationTypeName:selected?.typeName||formationEvaluationTypeDisplayName(f),scorePolicy:diag.selectedTypeId?'formation-effect-score-selected-type':'formation-effect-score-candidates',memberCount:0,candidateScores:diag.candidateScores||[],emptyReason:diag.emptyReason||''}};}
...
function renderFormationScoreSummaryHtml(f,data){
  ...
  return `<details class="formation-score-summary" aria-label="自動計算スコア"${openAttr}><summary class="formation-score-summary-head"><div class="formation-score-total"><span>トータルスコア</span><strong>${esc(scores.totalScore)}</strong></div><span class="formation-score-toggle-note">評価スコアを表示</span></summary><div class="formation-score-summary-body"><div class="formation-score-breakdown formation-score-generals">${rows.map(row=>`<span class="formation-score-chip"><span class="formation-score-label">${esc(row.label)}</span><span class="value">${esc(row.score)}</span></span>`).join('')}</div>${candidateRows}<div class="formation-score-breakdown formation-score-meta">${meta.map(([label,value])=>`<span><span class="formation-score-label">${esc(label)}</span>${esc(value)}</span>`).join('')}</div>${emptyNote}</div></details>`;
}
