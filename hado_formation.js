...
function calculateFormationAutoScores(f,data={}){const diag=calculateFormationTypeScore(f,data);const selected=diag.candidateScores?.[0]||null;const scoreRows=(selected?.rows||[]).slice(0,5).map(row=>({label:row.label,score:row.score,unit:'件',matchedEffects:Array.isArray(row.matchedEffects)?row.matchedEffects:[],matchedParameters:Array.isArray(row.matchedParameters)?row.matchedParameters:[]}));const totalScore=selected?Number(selected.totalScore||0):0;return {totalScore,evaluationScore:totalScore,breakdown:{scoreRows,filledGenerals:countFormationFilledGenerals(f),skillCount:Array.isArray(data?.skills)?data.skills.length:0,parameterCount:diag.effectSourceCount||diag.parameterRowCount||0,evaluationTypeName:selected?.typeName||formationEvaluationTypeDisplayName(f),scorePolicy:diag.selectedTypeId?'formation-effect-score-selected-type':'formation-effect-score-candidates',memberCount:0,candidateScores:diag.candidateScores||[],emptyReason:diag.emptyReason||''}};}
...
function renderFormationScoreMetricChipsHtml(rows,selectedIndex){return `<div class="formation-score-metric-chips" role="tablist" aria-label="評価スコア項目">${rows.map((row,index)=>`<button type="button" class="formation-score-metric-chip ${index===selectedIndex?'is-active':''} ${Number(row.score||0)>0?'':'is-zero'}" data-formation-score-detail-index="${esc(index)}" aria-pressed="${index===selectedIndex?'true':'false'}"><span>${esc(row.label)}</span><strong>${esc(row.score)}点</strong></button>`).join('')}</div>`;}
function renderFormationScoreEvidencePanelHtml(row){if(!row)return '';const evidenceRows=normalizeFormationScoreEvidenceRows(row);const score=Number(row.score||0);const expanded=!!state.formationScoreDetailExpanded;const limit=expanded?30:8;const visible=evidenceRows.slice(0,limit);const hidden=Math.max(0,evidenceRows.length-visible.length);const body=visible.length?`<div class="formation-score-evidence-list">${visible.map(item=>`<div class="formation-score-evidence-item"><span class="formation-score-evidence-point">+${esc(item.point)}点</span><span class="formation-score-evidence-title" title="${esc(item.title)}">${esc(item.title)}</span><span class="formation-score-evidence-value">${esc(item.value)}</span><span class="formation-score-evidence-source" title="${esc(item.source)}">発生元：${esc(item.source)}</span><span class="formation-score-evidence-condition">条件：${esc(item.condition||'常に')}</span></div>`).join('')}</div>`:'<div class="formation-score-empty">該当なし</div>';const more=evidenceRows.length>8?`<button type="button" class="formation-score-show-more" data-formation-score-show-more="1">${expanded?'表示を絞る':`さらに表示（ほか${esc(Math.max(0,evidenceRows.length-8))}件）`}</button>`:'';const overflow=hidden>0&&expanded?`<div class="formation-score-more-note">ほか${esc(hidden)}件</div>`:'';return `<div class="formation-score-detail-panel"><div class="formation-score-detail-head"><strong>${esc(row.label)}の内訳</strong><span>${esc(score)}点 / ${esc(score)}件一致</span></div>${body}${more}${overflow}</div>`;}
function setFormationScoreDetailIndex(index){const n=Number(index);state.formationScoreDetailIndex=Number.isInteger(n)&&n>=0?n:0;state.formationScoreDetailExpanded=false;debugLog('formationScoreDetail:click',{index:state.formationScoreDetailIndex});renderFormationScreen();}
...
function renderFormationScoreSummaryHtml(f,data){
  ...
  const emptyNote=scores.breakdown.emptyReason?`<div class="formation-note">${esc(scores.breakdown.emptyReason)}</div>`:'';
  return `<details class="formation-score-summary" aria-label="自動計算スコア"${openAttr}><summary class="formation-score-summary-head"><div class="formation-score-total"><span>トータルスコア</span><strong>${esc(scores.totalScore)}</strong><small>${esc(metaText)}</small></div><span class="formation-score-toggle-note">評価スコアを表示</span></summary><div class="formation-score-summary-body">${metricChips}${detailPanel}${candidateRows}<div class="formation-score-breakdown formation-score-meta">${meta.map(([label,value])=>`<span><span class="formation-score-label">${esc(label)}</span>${esc(value)}</span>`).join('')}</div>${emptyNote}</div></details>`;
}
