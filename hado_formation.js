...
function renderFormationScoreEvidenceItemsHtml(evidenceRows,expanded=false){const rows=expanded?(evidenceRows||[]):uniqueFormationScoreEvidenceSummaryRows(evidenceRows);return rows.map(item=>expanded?`<div class="formation-score-evidence-detail-card is-${esc(item.kind||'type')}"><div class="formation-score-evidence-detail-line"><b>標準効果:</b><span class="formation-score-evidence-label">${esc(item.displayTitle||item.title)}</span></div><div class="formation-score-evidence-detail-line"><b>対象:</b><span>${esc(item.targetScopeLabel||'対象不明')}</span></div><div class="formation-score-evidence-detail-line"><b>根拠:</b><span>${esc(item.source||'スコア根拠')}</span></div>${formationScoreRawEffectLine(item)}</div>`:`<span class="formation-score-evidence-tag formation-quick-summary-chip is-summary is-${esc(item.kind||'type')}"><span class="formation-score-evidence-label">${esc(item.simpleTitle||formationScoreEvidenceSimpleTitle(item))}</span></span>`).join('');}
