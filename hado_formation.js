...
function renderFormationScoreExcludedSummaryHtml(row,expanded=false){const excluded=Array.isArray(row?.excludedRows)?row.excludedRows:[];if(!excluded.length)return '';const examples=FORMATION_SCORE_VACCINE_EXAMPLE_LABELS.map(label=>`<li>${esc(label)}</li>`).join('');if(!expanded)return `<div class="formation-score-zero-primary is-summary" data-formation-score-zero-primary="1"><p><strong>ワクチン型の加点対象なし</strong></p><p>この部隊には、ワクチン型として加点できる不利対策が見つかりません。</p></div>`;return `<div class="formation-score-zero-primary is-detail" data-formation-score-zero-primary="1"><p><strong>加点対象なし</strong></p><p>この項目では、採用された評価根拠だけを表示します。現在の部隊ではワクチン型として評価できる不利対策が見つかりません。</p><p class="formation-note">評価される例</p><ul>${examples}</ul></div>`;}
