...
function formationScoreRawEffectLine(item){const raw=String(item?.rawEffectText||item?.rawText||item?.matchedText||'').trim();return raw?`<span class="formation-score-evidence-raw">原文: ${esc(raw)}</span>`:'';}
...
function renderFormationScoreEvidenceItemsHtml(evidenceRows,expanded=false){const rows=expanded?(evidenceRows||[]):uniqueFormationScoreEvidenceSummaryRows(evidenceRows);return rows.map(item=>expanded?`<span class="formation-score-evidence-tag formation-quick-summary-chip is-${esc(item.kind||'type')}"><b>${esc(item.kindLabel||'標準効果')}</b><span class="formation-score-evidence-label">${esc(item.displayTitle||item.title)}</span><span class="formation-score-evidence-meta">対象: ${esc(item.targetScopeLabel||'対象不明')} / 根拠: ${esc(item.source||'スコア根拠')}</span>${formationScoreRawEffectLine(item)}</span>`:`<span class="formation-score-evidence-tag formation-quick-summary-chip is-summary is-${esc(item.kind||'type')}"><span class="formation-score-evidence-label">${esc(item.simpleTitle||formationScoreEvidenceSimpleTitle(item))}</span></span>`).join('');}
