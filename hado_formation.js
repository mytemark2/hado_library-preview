...
function createFormationScoreEvidence(row,index,sourceKind){const rawText=String(row?.rawText||row?.matchedText||[row?.key,row?.label,row?.group,row?.timing,row?.condition,row?.sourceLabel,row?.value].filter(Boolean).join(' '));const label=String(row?.label||row?.key||row?.statusEffectName||'');const family=inferFormationScoreEvidenceFamily({...row,rawText,label});if(!family)return null;const sourceLabel=String(row?.sourceLabel||row?.sourceItemName||row?.source||sourceKind||'');const timing=normalizeFormationScoreEvidenceTiming(row?.timing||row?.timingLabel||'');const targetScope=inferFormationScoreEvidenceTargetScope({...row,rawText,label});const sourceType=sourceKind==='parameter'?'formation':(row?.sourceType||row?.sourceItemCategory||'skill');const sourceId=String(row?.sourceId||row?.featureId||sourceLabel||`${sourceKind}:${index}`);const key=[sourceLabel,row?.key||label,rawText,family].map(norm).join('|');return {evidenceId:`formation:${sourceKind}:${index}:${family}:${norm(sourceId)}`,sourceType,sourceId,sourceLabel,timing,targetScope,effectFamily:family,rawText,matchedText:rawText,isPrimaryEffect:true,isDerivedTag:false,isAggregateMetric:false,evidenceGroupKey:key,sourceKind:sourceKind==='parameter'?'parameter':'effect',label:label||family,key:row?.key||label,value:row?.value||'',condition:row?.condition||''};}
...
function buildFormationScoreEvidence(f,data={}){const warnings=[];const rows=[];const effects=Array.isArray(data?.effects)?data.effects:[];effects.forEach((effect,index)=>{const ev=createFormationScoreEvidence(effect,index,'effect');if(ev)rows.push(ev);});const parameterRows=[...collectFormationAllParameterRows(data),...collectFormationParameterCalculationRows(data)];parameterRows.forEach((row,index)=>{const label=row?.label||row?.key||'';const rawText=row?.rawText||row?.matchedText||`部隊の${label} ${row?.value||''}`;const ev=createFormationScoreEvidence({...row,rawText,label},index,'parameter');if(ev)rows.push(ev);});const candidateBuild=buildFormationCandidateScoreEvidence(f,data);candidateBuild.scoreEvidence.forEach(ev=>rows.push(ev));if(!effects.length)warnings.push('formationData.effects is empty');if(!parameterRows.length)warnings.push('parameterCalculation rows are empty');if(!candidateBuild.scoreEvidence.length)warnings.push('typeSearchFeatureIndex candidate evidence is empty');const seen=new Set(),deduped=[];rows.forEach(row=>{const key=row.evidenceGroupKey||row.evidenceId;if(seen.has(key))return;seen.add(key);deduped.push(row);});return {scoreEvidence:deduped,warnings,sources:{effects:effects.length,parameterRows:parameterRows.length,candidateRows:candidateBuild.rows.length,candidateEvidence:candidateBuild.scoreEvidence.length},formationCandidateEvidenceRows:candidateBuild.rows,candidateToScoreMismatches:candidateBuild.mismatches};}
...
function formationScoreSourceDisplayLabel(sourceLabel,sourceType=''){const raw=String(sourceLabel||'').trim();if(!raw||raw==='スコア根拠')return raw||'スコア根拠';const parts=raw.split(':').map(v=>v.trim()).filter(Boolean);const owner=parts[0]||raw;const kind=parts.find(v=>/戦法|技能|装備|陣形|五行|軍馬|異文化/.test(v))||({'skill':'技能','tactic':'戦法','equipment':'装備','formation':'陣形','fiveElement':'五行','warhorseSkill':'軍馬技能','ethnicResearchSkill':'異文化調査技能'}[sourceType]||'');return kind?`${owner} / ${kind}`:owner;}
function formationScoreRawEffectLine(item){const raw=String(item?.rawEffectText||item?.rawText||item?.matchedText||'').trim();return raw?`<div class="formation-score-evidence-raw"><span class="formation-score-evidence-raw-label">原文</span><p>${esc(raw)}</p></div>`:'';}
...
function renderFormationScoreEvidenceItemsHtml(evidenceRows,expanded=false){const rows=expanded?(evidenceRows||[]):uniqueFormationScoreEvidenceSummaryRows(evidenceRows);return rows.map((item,index)=>expanded?`<article class="formation-score-evidence-row is-${esc(item.kind||'type')}"><div class="formation-score-evidence-row-head"><span class="formation-score-evidence-index">${esc(index+1)}</span><span class="formation-score-evidence-effect formation-score-evidence-label">${esc(item.displayTitle||item.title)}</span></div><dl class="formation-score-evidence-meta"><div><dt>対象</dt><dd>${esc(item.targetScopeLabel||'対象不明')}</dd></div><div><dt>根拠</dt><dd>${esc(item.source||'スコア根拠')}</dd></div></dl>${formationScoreRawEffectLine(item)}</article>`:`<span class="formation-score-evidence-tag formation-quick-summary-chip is-summary is-${esc(item.kind||'type')}"><span class="formation-score-evidence-label">${esc(item.simpleTitle||formationScoreEvidenceSimpleTitle(item))}</span></span>`).join('');}
...
const FORMATION_SCORE_EXCLUDE_REASON_LABELS={deny_change_item:'この型では評価しない項目',target_not_allowed:'対象がこの型の条件外',unknown_target_for_target_dependent_type:'対象が判定できないため除外',duplicate_evidence:'同一起源の重複根拠',support_without_primary:'Primary不足のため補助効果のみでは評価不可',source_type_not_allowed:'根拠種別がこの型の条件外',timing_not_allowed:'発動タイミングがこの型の条件外',aggregate_or_derived_origin:'集計・派生根拠のため直接評価対象外'};
