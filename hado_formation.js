...
function renderFormationScoreEvidencePanelHtml(row){if(!row)return '';const evidenceRows=normalizeFormationScoreEvidenceRows(row);const score=Number(row.score||0);const expanded=!!state.formationScoreDetailExpanded;const limit=expanded?30:8;const visible=evidenceRows.slice(0,limit);const hidden=Math.max(0,evidenceRows.length-visible.length);const body=visible.length?`<div class="formation-score-evidence-list">${visible.map(item=>`<div class="formation-score-evidence-item"><span class="formation-score-evidence-point">+${esc(item.point)}点</span><span class="formation-score-evidence-title" title="${esc(item.title)}">${esc(item.title)}</span><span class="formation-score-evidence-value">${esc(item.value)}</span><span class="formation-score-evidence-source" title="${esc(item.source)}">発生元：${esc(item.source)}</span><span class="formation-score-evidence-condition">条件：${esc(item.condition||'常に')}</span></div>`).join('')}</div>`:`<div class="formation-score-empty"><strong>一致根拠なし</strong><span>${esc(row.label)}に一致した技能・効果・変化率はありません。</span></div>`;const more=evidenceRows.length>8?`<button type="button" class="formation-score-show-more" data-formation-score-show-more="1">${expanded?'表示を絞る':`さらに表示（ほか${esc(Math.max(0,evidenceRows.length-8))}件）`}</button>`:'';const overflow=hidden>0&&expanded?`<div class="formation-score-more-note">ほか${esc(hidden)}件</div>`:'';return `<div class="formation-score-detail-panel"><div class="formation-score-detail-head"><strong>${esc(row.label)}の内訳</strong><span>${esc(score)}点 / ${esc(score)}件一致</span></div>${body}${more}${overflow}</div>`;}
