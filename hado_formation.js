...
function renderFormationQuickResultSummary(data,formation=null){const rows=collectFormationDecisionSummaryRows(data,formation);return `<div class="formation-quick-summary-strip formation-result-enlarge-strip formation-decision-summary" data-formation-summary-open="1" role="button" tabindex="0"><div class="formation-quick-summary-title"><span>結果サマリー</span><span class="note">編成による加算値・タップで根拠表示</span></div><div class="formation-decision-summary-grid">${rows.map(renderFormationDecisionSummaryCard).join('')}</div></div>`;}
...
function buildFormationSummaryDialogHtmlFromRows(rows){return `<div class="formation-mobile-dialog-overlay formation-summary-dialog-overlay" data-formation-dialog-backdrop="summary"><div class="formation-mobile-dialog-card" role="dialog" aria-modal="true" aria-label="結果サマリー"><div class="formation-mobile-dialog-head"><div><div class="formation-mobile-dialog-title">結果サマリー</div><div class="formation-note">最終能力値ではなく、現在編成から確認できる加算値です。</div></div><button type="button" class="formation-mobile-dialog-close" data-formation-summary-close="1">閉じる</button></div><div class="formation-mobile-summary-dialog-body"><div class="formation-decision-summary-dialog-list">${rows.map(row=>`<section class="formation-decision-summary-dialog-card"><h3>${esc(row.label)}</h3>${(row.parts||[]).map(part=>`<div class="formation-decision-summary-dialog-row"><div>${renderFormationDecisionSummaryPart(part)}<span class="formation-decision-summary-scope">${esc(part.scopeLabel||'-')}</span></div><div class="formation-note">${esc(part.detail||'根拠なし')}</div></div>`).join('')}</section>`).join('')}</div><div class="formation-note">固定値と割合、常時と条件付きは合算せずに表示しています。詳細な計算式は「変化率」タブで確認してください。</div></div></div></div>`;}
...
function renderFormationConditionPanelHtml(result){
  ...
  const reviewedRows=(result.rows||[]).map(row=>`<article class="formation-condition-row is-${esc(row.state)}"><div class="formation-condition-row-head"><span class="formation-condition-badge is-${esc(row.state)}">${esc(row.label)}</span><strong>${esc(formationGeneralNameNoReading(row.sourceName))}</strong><span class="formation-condition-role">${esc(row.sourceRole)}</span></div><div class="formation-condition-case">${esc(row.caseId)}</div><details><summary>判定根拠の原文</summary><div class="formation-condition-raw">${esc(row.rawText)}</div></details></article>`).join('');
  const unresolvedRows=(result.unresolved||[]).map(row=>`<article class="formation-condition-row is-unknown"><div class="formation-condition-row-head"><span class="formation-condition-badge is-unknown">判定不可</span><strong>${esc(formationGeneralNameNoReading(row.sourceName))}</strong><span class="formation-condition-role">${esc(row.sourceRole)}</span></div><div class="formation-condition-unresolved">未確認の生成Clause ${esc(row.count)}件は推測判定せず、正式reviewed Clauseの追加待ちです。</div></article>`).join('');
  const empty=reviewedRows||unresolvedRows?'':'<div class="detail-empty">編成済み武将に条件Clauseがありません。</div>';
  return `<section class="formation-condition-shell"><div class="formation-condition-header"><div><h3>編成条件の評価</h3><p>現在の編成から確定できる条件だけを判定します。戦闘中の兵力・敵能力・状態変化は「戦闘中判定」、任命は「対象外」、未確認データは「判定不可」です。</p></div><div class="formation-condition-contract">reviewed ${esc(result.status?.reviewedCaseCount||0)} / Clause ${esc(result.status?.clauseCount||0)}</div></div><div class="formation-condition-states">${renderFormationConditionStateChips(result.counts)}</div><div class="formation-condition-list">${reviewedRows}${unresolvedRows}${empty}</div></section>`;
}
function injectFormationConditionPanel(){if(!els.formationRoot)return;const detailPanel=els.formationRoot.querySelector('.formation-detail-panel');if(!detailPanel)return;const evaluator=window.HADO_FORMATION_CONDITION_EVALUATOR;let result={ok:false,error:'Evaluatorが読み込まれていません。',rows:[],unresolved:[],counts:{unknown:1}};if(evaluator){try{result=evaluator.evaluateFormation(buildFormationConditionSnapshot(getCurrentFormation()));}catch(err){result={ok:false,error:err?.message||String(err),rows:[],unresolved:[],counts:{unknown:1}};}}state.diagnostics.formationConditions={timestamp:debugTimestamp(),...result,rows:(result.rows||[]).map(row=>({caseId:row.caseId,sourceName:row.sourceName,sourceRole:row.sourceRole,state:row.state,reason:row.reason})),unresolved:result.unresolved||[]};const active=normalizeFormationInnerTab(state.formationInnerTab);const panel=document.createElement('section');panel.className=`formation-work-tab-panel formation-detail-focus formation-condition-focus ${active==='condition'?'':'tab-content-hidden'}`;panel.dataset.formationWorkPanel='condition';panel.id='formation-work-panel-condition';panel.setAttribute('role','tabpanel');panel.setAttribute('aria-labelledby','formation-work-tab-condition');panel.innerHTML=renderFormationConditionPanelHtml(result);detailPanel.appendChild(panel);applyFormationInnerTabDisplay(active);debugLog('formationCondition:evaluated',{ok:!!result.ok,counts:result.counts||{},reviewedRows:(result.rows||[]).length,unresolvedSources:(result.unresolved||[]).length,persisted:false});}
