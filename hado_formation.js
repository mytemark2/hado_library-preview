...
function calculateFormationAutoScores(f,data={}){const generalRows=calculateFormationGeneralScoreRows(f,data);const totalScore=generalRows.reduce((sum,row)=>sum+row.score,0);const filledRows=generalRows.filter(row=>row.name&&row.name!=='未設定');const evaluationScore=filledRows.reduce((sum,row)=>sum+row.score,0);const skillCount=Array.isArray(data?.skills)?data.skills.length:0;const parameterCount=Object.keys(data?.parameterSummary||{}).length||0;return {totalScore,evaluationScore,breakdown:{generalRows,filledGenerals:filledRows.length,skillCount,parameterCount,evaluationTypeName:formationEvaluationTypeDisplayName(f)}};}
function renderFormationScoreSummaryHtml(f,data){const scores=calculateFormationAutoScores(f,data);f.totalScore=scores.totalScore;f.evaluationScore=scores.evaluationScore;const rows=scores.breakdown.generalRows;const meta=[['型',scores.breakdown.evaluationTypeName],['合算技能',`${scores.breakdown.skillCount}件`],['状態変化',`${scores.breakdown.parameterCount}件`]];return `<div class="formation-score-summary" aria-label="自動計算スコア"><div class="formation-score-total"><span>トータルスコア</span><strong>${esc(scores.totalScore)}点</strong></div><div class="formation-score-sub"><span>評価スコア</span><strong>${esc(scores.evaluationScore)}点</strong></div><div class="formation-score-breakdown formation-score-generals">${rows.map(row=>`<span><span class="formation-score-label">${esc(row.label)}</span>${esc(row.name)}: ${esc(row.score)}点</span>`).join('')}</div><div class="formation-score-breakdown formation-score-meta">${meta.map(([label,value])=>`<span><span class="formation-score-label">${esc(label)}</span>${esc(value)}</span>`).join('')}</div></div>`;}
