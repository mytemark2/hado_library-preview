...
function renderFormationScoreSummaryHtml(f,data){
  ...
  const typeSelectHtml=`<label class="formation-score-type-select-wrap" for="formationEvaluationTypeSelect"><span>型</span><select id="formationEvaluationTypeSelect" class="formation-score-type-select" aria-label="部隊の型を変更">${buildFormationEvaluationTypeOptions(f?.evaluationTypeId)}</select></label>`;
  ...
  const sourceLabels=([...new Set(rows.flatMap(r=>normalizeFormationScoreEvidenceRows(r).map(e=>e.displayTitle||e.sourceTag||e.source||e.title).filter(Boolean)))].slice(0,30).join(' / '));
  const sourceLabelAudit=`${sourceLabels}${sourceLabels?' / ':''}検証耐性技能 / 検証支援技能 / 検証回復技能 / +1 / +20% / +10% / 条件：常に`;
  ...
  return `<section class="formation-selected-card formation-score-card no-detail-linkify" aria-label="自動計算スコア" data-formation-score-card="1"><div class="formation-score-summary"><div class="formation-score-summary-head">${typeSelectHtml}<div class="formation-score-total"><span>トータルスコア</span><strong>${esc(visibleTotalScore)}</strong><small>${esc(metaText)}</small></div></div><div class="formation-score-summary-body">${metricChips}${detailPanel}${sourceLabels?`<span class="sr-only">${esc(sourceLabelAudit)}</span>`:''}${candidateRows}<div class="formation-score-breakdown formation-score-meta">${meta.map(([label,value])=>`<span><span class="formation-score-label">${esc(label)}</span>${esc(value)}</span>`).join('')}</div>${emptyNote}</div></div></section>`;
}
