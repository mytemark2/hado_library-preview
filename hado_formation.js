...
function formationScoreExcludedReasonCounts(rows){return (Array.isArray(rows)?rows:[]).reduce((acc,row)=>{const key=row?.excludeReason||'unknown';acc[key]=(acc[key]||0)+1;return acc;},{});}
function renderFormationScoreExcludedSummaryHtml(row,expanded=false){const excluded=Array.isArray(row?.excludedRows)?row.excludedRows:[];if(!excluded.length)return '';const counts=formationScoreExcludedReasonCounts(excluded);const deny=counts.deny_change_item||0;const list=excluded.slice(0,expanded?10:5).map(item=>`<li><b>${esc(item.changeItemId||item.effectFamily||'除外')}</b> ${esc(item.label||item.key||'根拠')} / ${esc(item.sourceLabel||'出典不明')} / ${esc(item.excludeReason||'excluded')} / ${esc(item.targetScope||'unknown')}</li>`).join('');return `<div class="formation-score-zero-primary" data-formation-score-zero-primary="1"><p><strong>評価対象Primaryなし・除外根拠あり</strong></p><p>評価対象の不利対策Primaryはありません。防御・回復・戦法ゲージ・攻撃速度など、ワクチン型の混入禁止項目は除外されています。</p><p>除外件数: ${esc(excluded.length)} / deny_change_item: ${esc(deny)}</p><ul>${list}</ul></div>`;}
