...
function renderFormationScoreSummaryHtml(f,data){
  ...
  const openAttr=isResponsiveMobileMode()?'':' open';
  return `<details class="formation-score-summary" aria-label="自動計算スコア"${openAttr}><summary class="formation-score-summary-head"><div class="formation-score-total"><span>トータルスコア</span><strong>${esc(scores.totalScore)}</strong></div><span class="formation-score-toggle-note">評価スコアを表示</span></summary><div class="formation-score-summary-body"><div class="formation-score-breakdown formation-score-generals">${rows.map(row=>`<span class="formation-score-chip"><span class="formation-score-label">${esc(row.label)}</span><span class="value">${esc(row.score)}</span></span>`).join('')}</div><div class="formation-score-breakdown formation-score-meta">${meta.map(([label,value])=>`<span><span class="formation-score-label">${esc(label)}</span>${esc(value)}</span>`).join('')}</div></div></details>`;
}
